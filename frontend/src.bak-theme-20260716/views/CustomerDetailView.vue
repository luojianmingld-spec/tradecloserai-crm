      ...
      <div v-if="activeTab==='info'" class="tab-pane">
          ...
          <div class="form-grid">
            ...
            <div class="fg"><label>来源</label>
              <el-select v-model="editForm.source" style="width:100%" allow-create filterable default-first-option>
                <el-option label="WhatsApp" value="whatsapp" /><el-option label="展会" value="展会" />
                <el-option label="独立站询盘" value="独立站询盘" /><el-option label="主动开发" value="主动开发" />
                <el-option label="老客户推荐" value="老客户推荐" /><el-option label="手动添加" value="manual" />
              </el-select>
            </div>
          </div>
        ...
        <template v-else>
          ...
          <div class="info-group">
            ...
            <div class="info-row"><span class="ir-label">公司名</span><span class="ir-value">{{ customer.companyName || customer.company || '—' }}</span></div>
            <div class="info-row"><span class="ir-label">联系人</span><span class="ir-value">{{ customer.contactName || '—' }}</span></div>
            <div class="info-row"><span class="ir-label">职位</span><span class="ir-value">{{ customer.title || '—' }}</span></div>
            <div class="info-row"><span class="ir-label">国家</span><span class="ir-value"><span v-if="customer.country">{{ countryFlag(customer.country) }} {{ customer.country }}</span><span v-else class="muted">—</span></span></div>
            <div class="info-row"><span class="ir-label">地址</span><span class="ir-value">{{ customer.address || '—' }}</span></div>
            <div class="info-row"><span class="ir-label">行业</span><span class="ir-value">{{ customer.industry || '—' }}</span></div>
          </div>

          <div class="info-group">
            <div class="grp-title">💼 业务信息</div>
            <div class="info-row"><span class="ir-label">来源</span><span class="ir-value">{{ customer.source || '—' }}</span></div>
          ...
          <div class="info-group full-row req-module" :class="{ 'ai-hint': reqAiDirty }">
            ...
            <div v-if="reqEditing" class="req-form">
              ...
              <div class="req-actions">
                ...
                <el-button type="success" :loading="reqSaving" @click="saveRequirement" style="background:#00a884;border-color:#00a884;">💾 保存</el-button>
              </div>
            </div>
...
function fieldLabel(k){ return ({companyName:'公司名',contactName:'联系人',country:'国家',title:'职位',email:'邮箱',website:'网站',address:'地址',industry:'行业',source:'来源',notes:'备注'})[k] || k; }
...
<style scoped>
.cust-detail-page { flex:1; display:flex; flex-direction:column; background:#111b21; color:#e9edef; height:100%; overflow:hidden; min-width:0; }
.det-header { display:flex; align-items:center; gap:14px; padding:14px 20px; background:#202c33; border-bottom:1px solid #222d34; flex-shrink:0; flex-wrap:wrap; }
...
.cust-title { margin:0; font-size:18px; font-weight:600; }
.level-tag { padding:2px 10px; border-radius:10px; font-size:12px; font-weight:600; }
...
.cust-flag { font-size:15px; color:#8696a0; }
.contact-chip { color:#8696a0; font-size:13px; }
.h-row2 { font-size:13px; margin-top:4px; }
.muted { color:#8696a0; }
.spacer { flex:1; }
.header-actions { display:flex; gap:8px; }
.action-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 14px; border-radius:8px; border:1px solid #3b4a54; background:#2a3942; color:#e9edef; font-size:13px; cursor:pointer; min-height:44px; }
.action-btn:hover { background:#3b4a54; }
.wa-btn { background:#00a884 !important; border-color:#00a884 !important; color:#fff !important; }
.wa-btn:hover { background:#06cf9c !important; }
.email-btn { background:#7c3aed !important; border-color:#7c3aed !important; color:#fff !important; }
.email-btn:hover { background:#8b5cf6 !important; }

.det-tabs { display:flex; gap:4px; padding:0 20px; background:#202c33; border-bottom:1px solid #222d34; flex-shrink:0; overflow-x:auto; }
.det-tab { background:transparent; border:none; color:#8696a0; padding:12px 18px; cursor:pointer; font-size:14px; border-bottom:2px solid transparent; white-space:nowrap; }
.det-tab.active { color:#00a884; border-bottom-color:#00a884; }
.det-tab:hover:not(.active) { color:#e9edef; background:#2a3942; }

.det-body { flex:1; overflow-y:auto; overflow-x:hidden; padding:20px 28px; min-width:0; }
...
/* Info */
.info-group { background:#202c33; border-radius:8px; padding:14px 18px; margin-bottom:0; border:1px solid #222d34; }
.grp-title { font-weight:600; font-size:14px; margin-bottom:8px; color:#e9edef; }
.info-row { display:flex; padding:6px 0; border-bottom:1px solid #222d34; font-size:13px; gap:10px; align-items:center; }
.info-row:last-child { border-bottom:none; }
.ir-label { width:100px; color:#8696a0; flex-shrink:0; font-size:13px; }
.ir-value { flex:1; color:#e9edef; word-break:break-word; display:flex; align-items:center; gap:6px; flex-wrap:wrap; }
.link { color:#4FC3F7; text-decoration:none; }
.link:hover { text-decoration:underline; }
.mini-btn { background:#2a3942; border:1px solid #3b4a54; color:#e9edef; padding:3px 10px; border-radius:6px; cursor:pointer; font-size:12px; }
.mini-btn:hover { background:#3b4a54; }
.mini-btn.go { color:#00a884; border-color:#00a884; }
.mini-btn.go:hover { background:rgba(0,168,132,0.15); }
.mini-btn.danger { color:#f87171; border-color:#dc2626; }
.mini-btn.danger:hover { background:rgba(220,38,38,0.15); }
.notes-box { background:#111b21; padding:12px; border-radius:8px; color:#e9edef; font-size:14px; min-height:60px; white-space:pre-wrap; }

/* Edit panel */
.edit-panel { background:#202c33; border-radius:10px; padding:20px; border:1px solid #222d34; }
.panel-title { font-weight:600; font-size:15px; margin-bottom:16px; }
.form-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px 20px; }
.fg { display:flex; flex-direction:column; gap:4px; }
.fg label { color:#8696a0; font-size:13px; }
.fg-full { margin-top:12px; display:flex; flex-direction:column; gap:4px; }
.fg-full label { color:#8696a0; font-size:13px; margin-bottom:4px; }
.edit-actions { margin-top:16px; display:flex; gap:8px; justify-content:flex-end; }
:deep(.el-input__wrapper), :deep(.el-textarea__inner) { background:#2a3942 !important; box-shadow:none !important; }
:deep(.el-input__inner), :deep(.el-textarea__inner) { color:#e9edef !important; }

/* AI */
.ai-extract { display:flex; flex-direction:column; gap:10px; align-items:flex-start; }
.ai-desc { font-size:13px; }
.ai-btn { background:#7c3aed !important; border-color:#7c3aed !important; }
.ai-result { width:100%; background:#111b21; border-radius:8px; padding:12px; margin-top:8px; }
.ai-r-title { color:#8696a0; font-size:13px; margin-bottom:8px; }
.ai-r-row { display:flex; align-items:center; gap:8px; padding:6px 0; border-bottom:1px solid #222d34; font-size:13px; }
.ai-r-label { width:80px; color:#8696a0; flex-shrink:0; }
.ai-r-val { flex:1; color:#e9edef; }
...
.msg-bubble { max-width:75%; background:#202c33; padding:10px 14px; border-radius:10px; }
.msg-item.out .msg-bubble { background:#005c4b; }
.msg-meta { font-size:11px; color:#8696a0; margin-bottom:4px; }
.msg-body { font-size:14px; color:#e9edef; white-space:pre-wrap; word-break:break-word; }

/* Docs */
.docs-toolbar { display:flex; align-items:center; gap:12px; margin-bottom:14px; flex-wrap:wrap; }
.docs-title { font-size:16px; font-weight:600; }
.docs-kpi { font-size:13px; }
.new-doc-btn { background:#7c3aed !important; border-color:#7c3aed !important; }
.new-doc-btn:hover { background:#8b5cf6 !important; }
.doc-filter-tabs { display:flex; gap:6px; margin-bottom:12px; }
.df-tab { background:#202c33; border:1px solid #3b4a54; color:#8696a0; padding:6px 14px; border-radius:16px; font-size:13px; cursor:pointer; }
.df-tab.active { background:#7c3aed; color:#fff; border-color:#7c3aed; }
.df-tab:hover:not(.active):not(.disabled) { background:#2a3942; color:#e9edef; }
.df-tab.disabled { opacity:0.4; cursor:not-allowed; }
.doc-list { display:flex; flex-direction:column; gap:8px; }
.doc-card { background:#202c33; border:1px solid #222d34; border-radius:10px; padding:14px 16px; }
.doc-row1 { display:flex; align-items:center; gap:10px; }
.doc-num { font-weight:600; font-size:15px; }
.doc-status { padding:2px 8px; border-radius:10px; font-size:11px; font-weight:600; }
.doc-status.st-draft { background:rgba(134,150,160,0.2); color:#8696a0; }
.doc-status.st-sent { background:rgba(79,195,247,0.2); color:#4FC3F7; }
.doc-status.st-confirmed { background:rgba(0,168,132,0.2); color:#00a884; }
.doc-amount { font-weight:600; color:#f59e0b; }
.doc-row2 { font-size:13px; margin-top:4px; }
...
.timeline::before { content:''; position:absolute; left:8px; top:0; bottom:0; width:2px; background:#2a3942; }
.tl-item { position:relative; margin-bottom:16px; }
.tl-dot { position:absolute; left:-18px; top:6px; width:12px; height:12px; border-radius:50%; background:#8696a0; border:2px solid #111b21; }
.tl-dot.ai { background:#7c3aed; }
.tl-content { background:#202c33; border-radius:8px; padding:10px 14px; border:1px solid #222d34; }
.tl-meta { display:flex; align-items:center; gap:8px; font-size:12px; color:#8696a0; margin-bottom:6px; }
.tl-tag { padding:1px 8px; border-radius:8px; font-size:11px; }
.tl-tag.ai { background:rgba(124,58,237,0.2); color:#a78bfa; }
.tl-tag.manual { background:rgba(0,168,132,0.2); color:#00a884; }
.tl-text { font-size:14px; line-height:1.6; white-space:pre-wrap; }
...
/* ===== 需求总结模块 ===== */
.req-summary-box { background:#111b21; border-radius:8px; padding:10px 12px; font-size:13px; color:#8696a0; line-height:1.6; margin-top:8px; word-break:break-word; }
...
.sec-title { font-size:12px; font-weight:600; color:#8696a0; }
.sec-content { color:#d1d7db; font-size:13px; line-height:1.6; padding-left:0; margin-top:2px; }
.sec-table { margin-bottom:10px; }
.sec-table:last-child { margin-bottom:0; }
.sec-rows { display:grid; grid-template-columns: 1fr; gap:4px; }
.sec-row { display:flex; justify-content:space-between; gap:10px; padding:5px 0; border-bottom:1px solid #1a242b; font-size:12px; }
.sec-row:last-child { border-bottom:none; }
.sec-label { color:#54656f; flex-shrink:0; }
.sec-val { color:#e9edef; text-align:right; flex:1; word-break:break-word; }
.sec-empty { color:#54656f; font-style:italic; text-align:center; padding:12px; font-size:12px; }
.req-module { border-color:#3b4a54; }
...
.req-updated { font-size:12px; }
.req-source { padding:1px 8px; border-radius:8px; font-size:11px; }
.req-source.src-manual { background:rgba(0,168,132,0.18); color:#34d399; }
.req-source.src-ai_summary { background:rgba(124,58,237,0.2); color:#a78bfa; }
.ai-req-btn { background:rgba(124,58,237,0.15); border:1px solid #7c3aed; color:#c4b5fd; }
.ai-req-btn:hover:not(:disabled) { background:rgba(124,58,237,0.3); color:#fff; }
.ai-req-btn:disabled { opacity:0.6; cursor:not-allowed; }
.spinner-mini { display:inline-block; width:10px; height:10px; border:2px solid rgba(196,181,253,0.3); border-top-color:#c4b5fd; border-radius:50%; animation:spin 0.8s linear infinite; margin-right:4px; vertical-align:-1px; }
@keyframes spin { to { transform:rotate(360deg); } }

.ai-highlight { background:rgba(245,158,11,0.12); border:1px solid #f59e0b; color:#fbbf24; padding:6px 12px; border-radius:6px; font-size:12px; margin-bottom:10px; }
.req-grid { display:grid; grid-template-columns:1fr 1fr; gap:10px 14px; }
.req-grid .fg label { display:block; font-size:12px; color:#8696a0; margin-bottom:4px; }
.req-grid .fg :deep(.el-input__wrapper) { background:#111b21; box-shadow:0 0 0 1px #2a3942 inset; }
.req-form .fg-full { margin-top:10px; }
.req-form .fg-full label { display:block; font-size:12px; color:#8696a0; margin-bottom:4px; }
.req-form .fg-full :deep(.el-textarea__inner) { background:#111b21; color:#e9edef; }
.req-actions { display:flex; justify-content:flex-end; gap:8px; margin-top:12px; }
.req-actions .el-button--success { background:#00a884 !important; border-color:#00a884 !important; }
.req-actions .el-button--success:hover { background:#06cf9c !important; border-color:#06cf9c !important; }

.req-view .req-row4 { display:grid; grid-template-columns:repeat(4, 1fr); gap:10px; margin-bottom:10px; }
.req-cell { background:#111b21; border-radius:6px; padding:8px 10px; min-height:48px; }
.rc-label { display:block; font-size:11px; color:#8696a0; margin-bottom:3px; }
.rc-value { display:block; font-size:13px; color:#e9edef; word-break:break-word; line-height:1.4; }
.req-summary-box { background:#111b21; border-radius:6px; padding:10px 12px; color:#d1d7db; font-size:13px; line-height:1.6; white-space:pre-wrap; }
...
.req-empty a { color:#00a884; cursor:pointer; text-decoration:none; }
