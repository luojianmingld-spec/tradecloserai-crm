<template>
  <div class="crm-layout" :class="{ 'platform-collapsed': platformCollapsed, 'mobile-view': isMobile }">
    <div v-if="waRestricted" class="wa-restricted-banner">
      <span class="rb-icon">&#9888;&#65039;</span>
      <span class="rb-text">WhatsApp风控冷却中，暂不主动发消息</span>
      <span class="rb-time">{{ restRemain }}</span>
      <button class="rb-close" @click="waRestricted=false">&times;</button>
    </div>
    <!-- ① 平台导航栏 -->
    ...
    <div v-if="activePlatform === 'communication'" class="comm-module" :class="{ 'email-active': activeChannel === 'email', 'drawer-open': isMobile && mobileDrawerOpen }">
        ...
        <aside class="col-chatlist" :class="{ collapsed: !isMobile && chatlistCollapsed, 'm-hidden': isMobile && mobileInConv }">
          ...
          <!-- ⚡ 5分钟首响提醒条（最醒目，最高优先级） -->
          <div
            v-if="activeChannel === 'whatsapp' && chatStore.isConnected && chatStore.followupCounts.firstResponse > 0"
            class="first-response-bar"
            @click="jumpToFirstResponse(0)"
            title="点击立即回复首位等待客户"
          >
            ...
            <button class="frb-btn" @click.stop="jumpToFirstResponse(0)">立即回复 ⚡</button>
          </div>
          <!-- 待跟进汇总条 -->
          <div
        ...
        <aside class="col-function-panel" :class="{ collapsed: !isMobile && panelCollapsed, mini: !isMobile && aiMiniMode && activePanel === 'ai', translating: !isMobile && activePanel === 'translate', aitalking: !isMobile && activePanel === 'aitalk', customering: !isMobile && activePanel === 'customer', requirementing: !isMobile && activePanel === 'requirement', documenting: !isMobile && activePanel === 'documents', 'm-panel': isMobile, 'm-panel-show': isMobile && mobilePanel }" :style="!isMobile && !panelCollapsed ? 'width:' + panelWidth + 'px' : ''">
          ...
          <div class="col-header panel-col-header" :class="{ 'ai-col-header': activePanel === 'ai' }">
            ...
            <button v-else class="col-toggle" @click="switchPanel(lastActivePanel || 'aitalk')" :title="lastActivePanel === 'translate' ? '翻译设置' : (lastActivePanel === 'customer' ? '客户背调' : (lastActivePanel === 'requirement' ? '需求总结' : (lastActivePanel === 'documents' ? '客户单证' : (lastActivePanel === 'aitalk' ? 'AI话术' : 'AI助手'))))">
              <span style="font-size:18px">{{ lastActivePanel === 'translate' ? '🌐' : (lastActivePanel === 'customer' ? '👤' : (lastActivePanel === 'requirement' ? '🎯' : (lastActivePanel === 'documents' ? '📄' : (lastActivePanel === 'aitalk' ? '💬' : '🤖')))) }}</span>
            </button>
          </div>
          <!-- AI 助手面板 -->
            ...
            <div ref="aitalkScrollRef" class="aitalk-scroll" @scroll="onAitalkScroll">
              ...
              <div v-if="aiMessages.length === 0 && !aitalkLoading" class="aitalk-welcome">
                ...
                <div class="aitalk-chips">
                  ...
                  <button class="aitalk-chip" @click="sendQuickPrompt('请把客户最新消息翻译成中文')">📝 翻译客户消息</button>
                  <button class="aitalk-chip" @click="sendQuickPrompt('请分析客户意图和关键信号')">💡 分析客户意图</button>
                  <button class="aitalk-chip" @click="sendQuickPrompt('基于沟通记录推荐下一步跟进策略')">📋 推荐跟进策略</button>
                </div>
              </div>
            ...
            <div class="aitalk-footer">
              ...
              <template v-else>
                <div class="aitalk-quick-chips">
                  <button class="aitalk-mini-chip" @click="appendInput('更正式')">更正式</button>
                  <button class="aitalk-mini-chip" @click="appendInput('更简洁')">更简洁</button>
                  <button class="aitalk-mini-chip" @click="appendInput('更热情')">更热情</button>
                  <button class="aitalk-mini-chip" @click="appendInput('翻译成阿拉伯语')">翻译成阿拉伯语</button>
                  <button class="aitalk-mini-chip" @click="appendInput('翻译成英语')">翻译成英语</button>
                </div>
...
function tryParseRepliesInText(text) {
  ...
  if (!cand) {
    ...
    if (f !== -1 && l !== -1 && l > f) {
      const s2 = text.slice(f, l + 1);
      if (/"replies"\s*:/.test(s2)) cand = s2;
    }
  }
  if (!cand) return null;
  try {
...
const emptyCustomer = () => ({
  ...
  email: '', website: '', phone: '', notes: '', source: '未知', customerLevel: 'C',
...
.logo-text { font-size: 15px; font-weight: 700; white-space: nowrap; color: #00a884; }
...
.acc-meta.online-text { color: #00a884; }
.wa-account-card {
  display: flex; align-items: center; gap: 10px;
...
.acc-disconnect-btn:hover { background: #ea433520; color: #ea4335; }
.wa-show-qr-btn {
  margin-top: 8px;
  width: 100%;
  padding: 8px;
...
.wa-show-qr-btn:hover { border-color: #00a884; color: #00a884; }
.wa-qr-inline {
  margin-top: 10px;
...
.wa-qr-hint {
  font-size: 10px; color: #8696a0; text-align: center; line-height: 1.4;
}
...
.filter-tab:hover { background: #2a3942; color: #e9edef; }
.filter-tab.active {
  background: #00a884;
  color: #111b21;
  font-weight: 500;
...
.filter-tab.active .filter-tab-badge { background: rgba(17,27,33,0.2); color: #111b21; }
...
.conv-followup-dot.fu-dot-followup { background: #00a884; }
.conv-followup-dot.fu-dot-reactivate { background: #53bdeb; }
...
.conv-followup-dot-inline.fu-dot-followup { background: #00a884; }
.conv-followup-dot-inline.fu-dot-reactivate { background: #53bdeb; }
...
.first-response-bar {
  ...
  padding: 10px 14px;
  border-radius: 10px;
  background: linear-gradient(135deg, #f15c6d 0%, #b9303f 100%);
  color: #fff;
  font-size: 13px;
  display: flex;
  align-items: center;
...
.conv-item.conv-first-response {
  border-left: 3px solid #f15c6d !important;
...
.followup-summary-bar:hover { background: #2a3942; }
.followup-summary-bar.active { border-color: #00a884; background: #1f3733; }
.fu-badge {
  display: inline-flex; align-items: center; gap: 4px;
  padding: 2px 8px; border-radius: 10px;
  background: #2a3942; color: #e9edef; font-weight: 500;
}
.fu-badge.fu-urgent { color: #f5c98d; }
.fu-badge.fu-followup { color: #7cd9b7; }
.fu-badge.fu-reactivate { color: #a5dbf3; }
.fu-empty { color: #8696a0; font-size: 12.5px; }
.fu-filter-on { color: #00a884; font-size: 12px; margin-left: auto; }
.conv-item.conv-fu { border-left: 3px solid transparent; }
.conv-item.conv-fu:has(.fu-dot-urgent) { border-left-color: #f5a623; }
.conv-item.conv-fu:has(.fu-dot-followup) { border-left-color: #00a884; }
.conv-item.conv-fu:has(.fu-dot-reactivate) { border-left-color: #53bdeb; }
...
.ai-title { flex: 1; display: flex; align-items: center; gap: 8px; padding-left: 4px; color: #e9edef; font-weight: 600; font-size: 13px; }
...
.ai-mini-tip { text-align: center; font-size: 11px; color: #8696a0; padding: 4px 0; }
.ai-mini-expand {
  margin-top: auto; padding: 5px; background: transparent;
  border: 1px dashed #3b4a54; color: #8696a0; border-radius: 6px;
  font-size: 11px; cursor: pointer;
}
.ai-mini-expand:hover { color: #00a884; border-color: #00a884; }
...
.ai-action-btn:hover:not(:disabled) { background: #2a3942; }
.ai-action-btn.hot {
  background: #00a884;
  border-color: #00a884;
  color: #111b21;
...
.ai-action-btn.active { background: #00a88430 !important; border-color: #00a884 !important; color: #00a884 !important; }
...
.ai-empty, .ai-welcome {
  margin: auto; text-align: center; color: #8696a0; padding: 24px 12px;
}
.ai-empty-icon { font-size: 36px; margin-bottom: 8px; opacity: 0.5; }
.ai-empty-text, .ai-welcome-desc { font-size: 12px; color: #8696a0; }
.ai-welcome-title { font-size: 13px; color: #d1d7db; margin-bottom: 4px; }
...
.arh-title { font-weight: 600; color: #00a884; font-size: 12px; }
.arh-ts { margin-left: auto; font-size: 11px; color: #667781; }

.ai-loading {
  display: flex; align-items: center; gap: 8px;
  padding: 16px 4px; color: #8696a0; font-size: 12px;
}
.ai-dots { display: inline-flex; gap: 3px; }
.ai-dots span {
  width: 6px; height: 6px; border-radius: 50%; background: #00a884;
...
.ai-error {
  ...
  font-size: 12px; border-left: 3px solid #ea4335;
}

.ai-bubble {
  background: #202c33; border-radius: 10px; padding: 12px;
  font-size: 13px; line-height: 1.65; color: #e9edef;
...
.ai-reply-card:hover { background: #00a88430; border-color: #00a884; }
.ai-reply-card.flashed { background: #00a884; border-color: #00a884; }
.ai-reply-card.flashed .arc-text { color: #fff; }
.arc-idx {
  position: absolute; top: 10px; left: 10px;
  font-size: 11px; color: #00a884; font-weight: 600;
}
.ai-reply-card.flashed .arc-idx { color: #fff; }
.arc-text {
  font-size: 13px; line-height: 1.6; color: #e9edef; white-space: pre-wrap;
...
.ais-label { flex-shrink: 0; color: #00a884; font-weight: 500; min-width: 78px; }
.ais-val { flex: 1; color: #d1d7db; }
...
.ait-label { font-size: 11px; color: #00a884; font-weight: 600; margin-bottom: 3px; }
.ait-val { font-size: 13px; color: #e9edef; line-height: 1.5; white-space: pre-wrap; word-break: break-word; }
.ait-trans { font-size: 14px; font-weight: 500; }
.ait-orig { padding-top: 8px; border-top: 1px solid #2a3942; }
.ait-orig .ait-val { color: #8696a0; font-size: 12px; }
.arc-fill-btn {
  align-self: flex-start;
  background: #00a88420; border: 1px solid #00a88460;
  color: #00a884; padding: 5px 12px; border-radius: 4px;
  font-size: 11px; cursor: pointer; transition: all 0.15s;
}
.arc-fill-btn:hover { background: #00a884; color: #fff; }
...
.arf-btn {
  background: transparent; border: 1px solid #3b4a54;
  color: #8696a0; padding: 5px 12px; border-radius: 6px;
  font-size: 11px; cursor: pointer; transition: all 0.15s;
}
.arf-btn:hover { color: #e9edef; border-color: #8696a0; }
.arf-toast { font-size: 11px; color: #00a884; animation: fadeIn 0.2s; }
...
.col-function-panel.translating { background: #111b21; border-left: 1px solid #222d34; }
.col-function-panel.aitalking { background: #111b21; border-left: 1px solid #222d34; }
.col-function-panel.aitalking .panel-col-header { background: #202c33; color: #e9edef; border-bottom: 1px solid #222d34; }
.col-function-panel.aitalking .panel-col-header .col-toggle { color: #8696a0; }
.col-function-panel.aitalking .panel-col-header .col-toggle:hover { background: #2a3942; color: #e9edef; }
.col-function-panel.customering { background: #111b21; border-left: 1px solid #222d34; }
.col-function-panel.customering .panel-col-header { background: #202c33; color: #e9edef; border-bottom: 1px solid #222d34; }
.col-function-panel.customering .ai-title { color: #e9edef; }
.col-function-panel.customering .panel-col-header .col-toggle { color: #8696a0; }
.col-function-panel.customering .panel-col-header .col-toggle:hover { background: #2a3942; color: #e9edef; }
.col-function-panel.customering .panel-col-header .col-toggle:disabled { opacity:.5; cursor:not-allowed; }
.col-function-panel.aitalking .ai-title { color: #e9edef; }
.col-function-panel.translating .panel-col-header { background: #202c33; color: #e9edef; border-bottom: 1px solid #222d34; }
.col-function-panel.translating .panel-col-header .col-toggle { color: #8696a0; }
.col-function-panel.translating .panel-col-header .col-toggle:hover { background: #2a3942; color: #e9edef; }
...
.panel-col-header.ai-col-header .ai-title { color: #e9edef; }
.col-function-panel.translating .ai-title { color: #e9edef; }
.ai-mini-toggle { margin-left: 2px; }
.col-function-panel .ai-body { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
.col-function-panel .ai-actions {
  display: flex; flex-wrap: wrap; gap: 6px;
  padding: 10px; border-bottom: 1px solid #222d34; background: #111b21;
}
.col-function-panel .ai-action-btn {
  flex: 1; min-width: 0;
  height: 34px; background: #202c33; color: #8696a0;
  border: none; border-radius: 6px; cursor: pointer;
  font-size: 12px; display: flex; align-items: center; justify-content: center;
  transition: all .15s; font-family: inherit; padding: 0 6px; white-space: nowrap;
}
.col-function-panel .ai-action-btn:hover { background: #2a3942; color: #e9edef; }
.col-function-panel .ai-action-btn.active { background: #00a88430; color: #00a884; }
.col-function-panel .ai-action-btn.hot { color: #00a884; }
.col-function-panel .ai-action-btn:disabled { opacity: .35; cursor: not-allowed; }

/* 翻译设置面板 body — dark WhatsApp theme */
.trans-panel-body { flex: 1; overflow-y: auto; padding: 16px; background: #111b21; }
.trans-panel { padding: 0; }
.trans-block { background: transparent; border-radius: 0; padding: 2px 0 6px; margin-bottom: 4px; border-bottom: 1px solid #222d34; }
.trans-block:last-of-type { border-bottom: none; }
.switch-row { display: flex; align-items: center; height: 48px; padding: 0 4px; }
.switch-label { flex: 1; font-size: 15px; color: #e9edef; font-weight: 500; }
.sub-row { display: flex; align-items: center; min-height: 40px; padding: 0 4px 0 24px; margin-top: 2px; }
.sub-label { width: 72px; font-size: 14px; color: #8696a0; flex-shrink: 0; }
.sub-select { flex: 1; }
.sub-help { color: #8696a0; font-size: 14px; margin-right: 6px; cursor: help; transition: color .15s; }
.sub-help:hover { color: #00a884; }
.switch-help { margin-left: 4px; }
.trans-divider { display: flex; align-items: center; margin: 16px 0; }
.trans-divider::before, .trans-divider::after { content: ''; flex: 1; height: 1px; background: #222d34; }
.trans-divider-text { padding: 0 12px; font-size: 12px; color: #8696a0; }
...
.cfg-color-input { flex: 1; height: 32px; border: 1px solid #3b4a54; border-radius: 4px; padding: 0 10px; font-size: 13px; color: #e9edef; background: #2a3942; outline: none; font-family: monospace; min-width: 0; }
.cfg-color-input:focus { border-color: #00a884; }
.cfg-color-picker { flex-shrink: 0; }
.trans-save-btn { width: 100%; height: 40px; margin-top: 16px; background: #00a884; color: #fff; border: none; border-radius: 6px; font-size: 15px; font-weight: 500; cursor: pointer; transition: background .15s; font-family: inherit; }
.trans-save-btn:hover { background: #069671; }
.trans-save-btn:disabled { background: #3b5a54; color: #8696a0; cursor: not-allowed; }

/* ⑦ 图标列 — dark WhatsApp theme */
.col-iconbar {
  width: 64px; background: #202c33;
  display: flex; flex-direction: column;
...
.ib-item { background: transparent; border: none; cursor: pointer; display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 8px; border-radius: 8px; color: #aebac1; width: 100%; font-family: inherit; transition: background .15s, color .15s; }
.ib-item:hover { background: #2a3942; }
.ib-item.active { color: #00a884; }
...
.ib-toggle { margin-top: auto; align-self: center; width: 28px; height: 28px; border: none; background: #2a3942; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #8696a0; margin-bottom: 12px; padding: 0; transition: background .15s, color .15s, transform .28s cubic-bezier(.4,0,.2,1); }
.ib-toggle:hover { background: #3b4a54; color: #e9edef; }
...
@media (max-width: 768px) {
  ...
  .col-function-panel.m-panel {
    ...
    box-shadow: -2px 0 16px rgba(0,0,0,0.4); background: #111b21;
  }
  .col-function-panel.m-panel.show { transform: translateX(0); }
  .col-function-panel.m-panel.translating { background: #111b21; }
}
...
.wa-connect-title { font-size: 13px; font-weight: 600; color: #e9edef; }
.wa-connect-desc { font-size: 11px; color: #8696a0; }
.wa-connect-btn {
  margin-top: 8px;
  padding: 6px 14px;
  background: #00a884; color: #fff;
...
.wa-connect-btn:hover { background: #06cf9c; }
.empty-list-hint {
  padding: 40px 10px;
  display: flex; flex-direction: column;
  align-items: center; justify-content: center;
...
.wa-col-btn:hover { background: #364147; color: #e9edef; }
...
.wa-empty-meta .status-text.connecting { color: #53bdeb; }
.wa-empty-meta .status-text.error { color: #ea4335; }
...
.wa-account-item .acc-meta.online-text { color: #00a884; }
...
.connect-btn-big:hover { background: #06cf9c; }
.ph-icon.wa-green { font-size: 72px; opacity: 1; color: #00a884; }
...
.ai-chat-icon-btn:hover:not(:disabled) { background: #2a3942; color: #00a884; }
...
.ai-insert-btn:hover { background: #00a884; color: #fff; }
.ai-insert-btn-secondary { color: #8696a0; }
.ai-insert-btn-secondary:hover { background: #2a3942; color: #e9edef; }
...
.ai-trans-result { color: #00a884; font-weight: 500; }
.ai-trans-actions {
  display: flex;
  gap: 6px;
  justify-content: flex-end;
...
.ai-section-title { font-size: 11px; color: #8696a0; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
.ai-reply-styles { padding: 0 2px; }
.ai-style-row { display: flex; gap: 6px; margin-top: 6px; }
.ai-style-card {
  flex: 1; padding: 8px 4px;
  background: #202c33; border: 2px solid transparent;
  border-radius: 8px; cursor: pointer;
...
.ai-style-card:hover:not(.active) { background: #2a3942; }
.ai-gen-btn {
  width: 100%; padding: 10px 16px;
  background: linear-gradient(90deg, #00a884, #00c896);
...
.ai-reply-list { display: flex; flex-direction: column; gap: 8px; }
.ai-reply-card {
  background: #202c33; border-radius: 8px; padding: 10px 12px;
  border-top-left-radius: 2px;
  box-shadow: 0 1px 0.5px rgba(0,0,0,.13);
  display: flex; flex-direction: column; gap: 8px;
}
.ai-error-card { opacity: 0.85; }
.ai-reply-text { font-size: 13px; color: #e9edef; line-height: 1.55; word-break: break-word; white-space: pre-wrap; }
.ai-reply-footer { display: flex; align-items: center; justify-content: space-between; }
.ai-reply-meta { display: flex; align-items: center; gap: 8px; font-size: 11px; color: #8696a0; }
...
.ai-score-bar { height: 6px; background: #2a3942; border-radius: 3px; overflow: hidden; }
...
.ai-stage-text { font-size: 14px; color: #e9edef; font-weight: 600; }
...
.ai-field-label { font-size: 11px; color: #8696a0; font-weight: 600; text-transform: uppercase; letter-spacing: 0.3px; }
.ai-field-value { font-size: 13px; color: #e9edef; line-height: 1.5; white-space: pre-wrap; word-break: break-word; }
...
.ai-summary-text { font-size: 13px; color: #e9edef; line-height: 1.6; white-space: pre-wrap; word-break: break-word; }

/* 欢迎提示（profile用） */
.ai-welcome-text { font-size: 13px; color: #8696a0; }
...
.ai-chat-input::placeholder { color: #8696a0; }
...
.ai-chat-send:hover:not(:disabled) { background: #06cf9c; }
.ai-chat-send:disabled { opacity: 0.4; cursor: not-allowed; background: #00a884; }
...
/* Resize handle dark */
.col-function-panel .ai-resize-handle { background: #222d34; }


/* ── AI 话术面板 v2 ── */
.aitalk-panel-body {
  flex: 1; display: flex; flex-direction: column;
  background: #111b21; overflow: hidden; height: 100%;
}

.aitalk-top-bar {
  display: flex; flex-direction: column; gap: 6px;
  padding: 12px 12px 8px; border-bottom: 1px solid #222d34;
}
.aitalk-top-bar .aitalk-top-actions { display: flex; align-items: center; gap: 6px; margin-left: auto; }
.aitalk-model-select { flex: 1; }
.aitalk-model-select .el-select__placeholder, .aitalk-model-select .el-select__selected-item { color: #e9edef !important; }
...
.aitalk-select-label { color: #8696a0; font-size: 12px; white-space: nowrap; min-width: 54px; }
.aitalk-lang-select { flex: 1; }
.aitalk-lang-select .el-select__placeholder, .aitalk-lang-select .el-select__selected-item { color: #e9edef !important; font-size: 13px; }


.aitalk-top-actions { display: flex; gap: 4px; }
.aitalk-mode-btn {
  width: 30px; height: 30px; border-radius: 6px; border: none;
  background: #2a3942; color: #8696a0; cursor: pointer; font-size: 14px;
  display: flex; align-items: center; justify-content: center; transition: all .2s ease;
}
.aitalk-mode-btn:hover { background: #3b4a54; color: #e9edef; }
.aitalk-mode-btn.active { background: rgba(0,168,132,.15); color: #00a884; }
.aitalk-clear-btn {
  width: 30px; height: 30px; border-radius: 6px; border: none;
  background: transparent; color: #8696a0; cursor: pointer; font-size: 14px;
  display: flex; align-items: center; justify-content: center; transition: all .2s ease;
}
...
.aitalk-scroll::-webkit-scrollbar-thumb { background: #2a3942; border-radius: 3px; }
...
.aitalk-bubble-user {
  background: #005c4b; color: #e9edef; border-radius: 8px 0 8px 8px; max-width: 80%;
}
.aitalk-bubble-ai {
  background: #202c33; color: #e9edef; border-radius: 0 8px 8px 8px; max-width: 95%;
}
.aitalk-msg-time { font-size: 10px; color: #8696a0; margin-top: 3px; padding: 0 4px; }
...
.aitalk-retry-btn {
  background: #2a3942; color: #e9edef; border: 1px solid #3b4a54; border-radius: 4px;
  padding: 4px 10px; font-size: 12px; cursor: pointer; font-family: inherit;
}
.aitalk-retry-btn:hover { background: #3b4a54; }
.aitalk-error-global {
...
.aitalk-reply-card-v2 {
  background: #202c33; border-radius: 8px; padding: 12px; margin-bottom: 8px;
  max-width: 100%; position: relative; transition: background .2s ease;
}
.aitalk-reply-card-v2:hover { background: #26353d; }
.aitalk-reply-num {
  position: absolute; top: 8px; right: 10px; color: #00a884; font-weight: 700;
...
.aitalk-reply-name { color: #00a884; font-weight: 700; font-size: 14px; }
.aitalk-reply-sep { color: #8696a0; }
.aitalk-reply-desc { color: #8696a0; font-size: 12px; }
.aitalk-reply-foreign {
  background: #111b21; border-left: 3px solid #00a884; border-radius: 4px; padding: 10px;
  color: #e9edef; font-size: 14px; line-height: 1.55; margin: 6px 0;
  white-space: pre-wrap; word-break: break-word;
}
.aitalk-reply-chinese { color: #8696a0; font-size: 13px; font-style: italic; line-height: 1.5; margin-bottom: 8px; }
.aitalk-reply-actions { display: flex; gap: 6px; margin-bottom: 8px; }
.aitalk-mini-btn {
  background: #2a3942; color: #8696a0; border: none; border-radius: 4px;
  padding: 4px 10px; font-size: 12px; cursor: pointer; font-family: inherit;
  transition: all .2s ease;
}
.aitalk-mini-btn:hover { background: #3b4a54; color: #e9edef; }
.aitalk-use-btn {
  width: 100%; background: #00a884; color: #fff; border: none; border-radius: 6px;
...
.aitalk-scene-title { color: #53bdeb; font-size: 12px; font-weight: 700; margin-bottom: 6px; letter-spacing: .5px; }
.aitalk-scene-text { color: #d1d7db; font-size: 13px; line-height: 1.7; }
/* ── 区块分隔标题 ── */
.aitalk-section-label {
  font-size: 12px; color: #8696a0; font-weight: 600;
...
.aitalk-design-psych {
  flex: 0 0 40%; color: #8696a0; font-size: 12.5px; line-height: 1.6;
  padding-right: 10px; border-right: 1px solid #2a3942;
}
.aitalk-design-strategy {
  flex: 1; color: #e9edef; font-size: 12.5px; line-height: 1.6; padding-left: 10px;
}
/* ── 补充建议 ── */
...
.aitalk-suggest-list li {
  color: #d1d7db; font-size: 13px; line-height: 1.7; padding: 3px 0;
  padding-left: 14px; position: relative;
}
.aitalk-suggest-list li::before {
  content: "•"; color: #00a884; position: absolute; left: 2px; top: 3px; font-weight: 700;
}
/* ── 底部提示 ── */
.aitalk-footer-hint {
  color: #8696a0; font-size: 12px; margin: 10px 2px 4px; line-height: 1.5;
}
/* ── 优化反馈区 ── */
.aitalk-refine-block {
...
.aitalk-refine-title { color: #8696a0; font-size: 12px; margin-bottom: 8px; }
.aitalk-refine-chips { display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 8px; }
.aitalk-refine-chip {
  background: #2a3942; color: #8696a0; border: 1px solid transparent; border-radius: 14px;
  padding: 4px 10px; font-size: 12px; cursor: pointer; font-family: inherit;
  transition: all .2s ease;
}
.aitalk-refine-chip:hover { border-color: #00a884; color: #e9edef; }
.aitalk-refine-chip.active { background: rgba(0,168,132,.15); color: #00a884; border-color: #00a884; }
.aitalk-refine-textarea {
  width: 100%; box-sizing: border-box;
  background: #2a3942; border: 1px solid #3b4a54; border-radius: 8px;
  padding: 8px 10px; color: #e9edef; font-size: 13px; resize: vertical;
  outline: none; font-family: inherit; line-height: 1.5; min-height: 60px;
  transition: border-color .2s ease; margin-bottom: 8px;
}
.aitalk-refine-textarea:focus { border-color: #00a884; }
.aitalk-refine-textarea::placeholder { color: #8696a0; }
...
.aitalk-refine-btn:disabled { background: #3b4a54; cursor: not-allowed; opacity: .8; }
.aitalk-refine-hint { color: #8696a0; font-size: 11.5px; margin-top: 6px; text-align: center; }


.aitalk-footer {
  padding: 10px 12px 12px; border-top: 1px solid #222d34; background: #111b21;
  display: flex; flex-direction: column; gap: 8px;
}
...
.aitalk-mini-chip:hover { background: #3b4a54; color: #e9edef; }
.aitalk-input-row { display: flex; gap: 8px; align-items: flex-end; }
.aitalk-textarea {
  flex: 1; background: #2a3942; border: 1px solid #3b4a54; border-radius: 8px;
  padding: 10px 12px; color: #e9edef; font-size: 14px; resize: none;
  outline: none; font-family: inherit; line-height: 1.4; min-height: 42px; max-height: 120px;
  transition: border-color .2s ease;
}
.aitalk-textarea:focus { border-color: #00a884; }
.aitalk-textarea::placeholder { color: #8696a0; }
.aitalk-send-btn {
  width: 38px; height: 38px; border-radius: 50%; background: #00a884; border: none;
  color: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center;
  flex-shrink: 0; transition: all .2s ease;
}
.aitalk-send-btn:hover:not(:disabled) { background: #008c6e; }
.aitalk-send-btn:disabled { background: #3b4a54; cursor: not-allowed; }
...
.aitalk-switch-mode-link {
  background: none; border: none; color: #8696a0; font-size: 12px; cursor: pointer;
  padding: 2px 0; font-family: inherit; text-align: center; transition: color .2s ease;
}
.aitalk-switch-mode-link:hover { color: #00a884; }

.aitalk-scroll-down {
  position: absolute; right: 16px; bottom: 8px; width: 32px; height: 32px;
  border-radius: 50%; background: #202c33; border: 1px solid #3b4a54; color: #00a884;
  cursor: pointer; display: flex; align-items: center; justify-content: center;
  font-size: 14px; box-shadow: 0 2px 8px rgba(0,0,0,.4); z-index: 5;
  transition: all .2s ease;
}
.aitalk-scroll-down:hover { background: #2a3942; }
...
.customer-header {
  height: 60px; background: #202c33; border-bottom: 1px solid #222d34;
  display: flex; align-items: center; padding: 0 14px; gap: 10px; flex-shrink: 0;
}
.customer-avatar {
  width: 40px; height: 40px; border-radius: 50%; background: #2a3942;
  color: #e9edef; display: flex; align-items: center; justify-content: center;
  font-size: 16px; font-weight: 600; flex-shrink: 0; text-transform: uppercase;
}
.customer-head-info { flex: 1; min-width: 0; }
.customer-head-name { color: #e9edef; font-size: 15px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.customer-head-jid { color: #8696a0; font-size: 12px; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.customer-head-actions { display: flex; align-items: center; gap: 6px; }
.customer-level-tag {
  display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 11px; font-weight: 600;
  background: #3b4a54; color: #8696a0;
}
...
.customer-level-tag.lv-C { background: rgba(134,150,160,.15); color: #8696a0; }
...
.customer-bg-btn {
  width: 100%; height: 40px; background: #00a884; color: #fff; border: none; border-radius: 6px;
...
.bg-check-time { font-size: 11px; color: #8696a0; text-align: center; padding: 0 12px; margin-top: -4px; }
.bg-report-card { background: #202c33; border: 1px solid #2a3942; border-radius: 10px; margin: 4px 12px 10px; overflow: hidden; }
.bg-report-head { display: flex; align-items: center; justify-content: space-between; padding: 10px 12px 8px; border-bottom: 1px solid #2a3942; }
.bg-report-body { padding: 10px 12px; font-size: 13px; line-height: 1.7; color: #d1d7db; max-height: 500px; overflow-y: auto; white-space: normal; }
.bg-report-body p { margin: 0 0 2px; }
.bg-report-body p.bg-report-empty { height: 4px; margin: 0; }
.bg-report-footer { padding: 8px 12px 12px; border-top: 1px solid #2a3942; display: flex; flex-direction: column; gap: 8px; }
.bg-missing-tip { font-size: 12px; color: #8696a0; }
...
.customer-ask-list::-webkit-scrollbar-thumb { background: #2a3942; border-radius: 3px; }
.customer-ask-card {
  background: #202c33; border-radius: 8px; padding: 10px 12px; border-left: 3px solid #f59e0b;
}
.customer-ask-name { color: #f59e0b; font-weight: 600; font-size: 12px; margin-bottom: 6px; }
.customer-ask-foreign {
  background: #111b21; border-radius: 4px; padding: 8px 10px; color: #e9edef; font-size: 13px;
  line-height: 1.5; white-space: pre-wrap; word-break: break-word; margin-bottom: 6px;
}
.customer-ask-chinese { color: #8696a0; font-size: 12px; font-style: italic; margin-bottom: 8px; }
.customer-ask-use {
  width: 100%; background: #2a3942; color: #e9edef; border: none; border-radius: 4px;
  padding: 7px; font-size: 12px; cursor: pointer; font-family: inherit; transition: background .2s;
}
.customer-ask-use:hover { background: #00a884; color: #fff; }

.customer-extract-card {
  background: #202c33; border: 1px solid #00a884; border-radius: 8px; padding: 12px;
  margin: 8px 12px;
}
...
.customer-extract-close {
  background: transparent; color: #8696a0; border: none; font-size: 14px; cursor: pointer;
}
.customer-extract-close:hover { color: #e9edef; }
.customer-extract-fields { display: flex; flex-direction: column; gap: 6px; }
.customer-extract-row {
  display: flex; align-items: center; gap: 8px; padding: 6px 0; border-bottom: 1px dashed #222d34; font-size: 13px;
}
.customer-extract-row:last-child { border-bottom: none; }
.customer-extract-label { color: #8696a0; min-width: 70px; font-size: 12px; }
.customer-extract-val { color: #e9edef; flex: 1; word-break: break-word; }
.customer-extract-adopt {
  background: #2a3942; color: #e9edef; border: none; border-radius: 4px; padding: 2px 8px;
  font-size: 11px; cursor: pointer; font-family: inherit; transition: all .2s;
}
.customer-extract-adopt:hover { background: #00a884; }
.customer-extract-exists { color: #8696a0; font-size: 11px; }
...
.customer-fields::-webkit-scrollbar-thumb { background: #2a3942; border-radius: 3px; }
.customer-field { padding: 10px 0; border-bottom: 1px solid #222d34; }
.customer-field:last-child { border-bottom: none; }
.customer-field.warn .customer-field-label { color: #f59e0b !important; }
.customer-field-label { color: #8696a0; font-size: 12px; margin-bottom: 4px; display: flex; align-items: center; gap: 4px; }
.customer-required { color: #ef4444; }
.customer-field-value {
  color: #e9edef; font-size: 14px; line-height: 1.5; display: flex; align-items: center; gap: 8px;
  word-break: break-word;
}
.customer-field-value.empty { color: #8696a0; font-style: italic; }
.customer-input :deep(.el-input__wrapper),
.customer-input :deep(.el-textarea__inner),
.customer-input :deep(.el-select__wrapper) {
  background: #2a3942 !important; color: #e9edef !important; border-radius: 4px !important;
  box-shadow: none !important; border: 1px solid #3b4a54 !important;
}
.customer-input :deep(.el-input__inner),
.customer-input :deep(.el-textarea__inner) {
  color: #e9edef !important; background: #2a3942 !important;
}
.customer-input :deep(.el-textarea__inner) { font-family: inherit; }
.customer-input :deep(.el-input__wrapper.is-focus),
.customer-input :deep(.el-textarea__inner:focus) { border-color: #00a884 !important; }
.customer-input :deep(.el-select__wrapper.is-focused) { border-color: #00a884 !important; }

.customer-footer {
  display: flex; gap: 8px; padding: 10px 14px; border-top: 1px solid #222d34; background: #111b21;
}
.customer-btn-primary {
  flex: 1; background: #00a884; color: #fff; border: none; border-radius: 6px; padding: 9px;
  font-size: 14px; font-weight: 500; cursor: pointer; font-family: inherit; transition: background .2s;
}
.customer-btn-primary:hover:not(:disabled) { background: #008c6e; }
.customer-btn-primary:disabled { opacity: .6; cursor: not-allowed; }
.customer-btn-ghost {
  background: #2a3942; color: #e9edef; border: none; border-radius: 6px; padding: 9px 16px;
  font-size: 14px; cursor: pointer; font-family: inherit; transition: background .2s;
}
.customer-btn-ghost:hover { background: #3b4a54; }
...
@media (max-width: 768px) {
  ...
  .mh-btn.active { background: #00a88430; color: #00a884; border-radius: 50%; }
  ...
  .workspace { flex: 1; min-height: 0; position: relative; width: auto; overflow: hidden; height: 0; padding-bottom: 62px; box-sizing: border-box; }
  .workspace.has-mheader { padding-top: 0; }
  /* comm-module/module-full在手机端必须占满workspace（workspace用flex:1+height:0技巧） */
  .comm-module, .module-full { position: absolute; inset: 0; width: 100%; height: 100%; }
  .comm-module { overflow: hidden; }
  .module-full { overflow-y: auto !important; -webkit-overflow-scrolling: touch; background: #111b21; }
  ...
  .col-function-panel.m-panel .m-panel-close:active { background: #2a3942; }
  ...
  .mobile-tabbar {
    ...
    background: #1a2329; border-top: 1px solid #2a3942;
  ...
  .settings-sidebar { width: 100% !important; height: auto !important; flex-direction: row; overflow-x: auto; flex-shrink: 0; border-right: none; border-bottom: 1px solid #222d34; }
...
.wa-restricted-banner{position:fixed;top:0;left:0;right:0;z-index:999999;background:linear-gradient(90deg,#d32f2f,#f44336);color:#fff;padding:8px 12px;display:flex;align-items:center;gap:8px;font-size:13px;font-weight:500;box-shadow:0 2px 8px rgba(0,0,0,.3);height:36px;box-sizing:border-box}
.rb-icon{font-size:14px}
.rb-text{flex:1;line-height:1.2}
.rb-time{background:rgba(255,255,255,.2);padding:2px 10px;border-radius:10px;font-size:12px;white-space:nowrap}
.rb-close{background:0 0;border:none;color:#fff;font-size:20px;cursor:pointer;padding:0 4px;line-height:1;opacity:.8}
.rb-close:hover{opacity:1}
@media(max-width:768px){
  .crm-layout{padding-top:0!important}
  .wa-restricted-banner{position:relative!important;top:auto!important;left:auto!important;right:auto!important;z-index:auto!important;font-size:12px;padding:6px 10px;height:auto!important;box-shadow:none!important;border-radius:0;flex-shrink:0}
  .rb-text{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.req-row { display: flex; align-items: flex-start; padding: 8px 0; border-bottom: 1px solid #222d34; gap: 10px; }
.req-row:last-of-type { border-bottom: none; }
.req-label { color: #8696a0; font-size: 12px; min-width: 78px; flex-shrink: 0; padding-top: 2px; }
.req-val { color: #e9edef; font-size: 13px; flex: 1; word-break: break-word; line-height: 1.5; }
.req-val.empty { color: #5a6a75; }
.req-summary-block { padding: 12px 0 4px; border-top: 1px solid #222d34; margin-top: 6px; }
.req-summary-text { background: #202c33; padding: 10px 12px; border-radius: 8px; color: #d1d7db; font-size: 13px; line-height: 1.6; min-height: 40px; white-space: pre-wrap; word-break: break-word; }
...
.req-summary-text-block.empty { color: #8696a0; font-style: italic; }
/* 需求分析傻瓜化样式 */
...
/* ── 需求分析Markdown渲染样式 ── */
.req-md-content { font-size: 13px; line-height: 1.7; color: #d1d7db; }
.req-md-content h2 { font-size: 16px; color: #e9edef; margin: 16px 0 10px; padding-bottom: 6px; border-bottom: 1px solid #2a3942; font-weight: 600; }
.req-md-content h2:first-child { margin-top: 0; }
.req-md-content h3 { font-size: 14px; color: #00a884; margin: 14px 0 8px; font-weight: 600; }
.req-md-content table { width: 100%; border-collapse: collapse; margin: 6px 0 10px; font-size: 12.5px; background: #202c33; border-radius: 6px; overflow: hidden; }
.req-md-content th { background: #2a3942; color: #8696a0; font-weight: 600; text-align: left; padding: 8px 10px; border-bottom: 1px solid #3b4a54; font-size: 12px; }
.req-md-content td { padding: 7px 10px; border-bottom: 1px solid #2a3942; color: #d1d7db; vertical-align: top; }
.req-md-content tr:last-child td { border-bottom: none; }
.req-md-content strong { color: #e9edef; font-weight: 600; }
.req-md-content ol, .req-md-content ul { padding-left: 20px; margin: 6px 0; }
.req-md-content li { margin: 4px 0; color: #d1d7db; }
.req-md-content ul li::marker { color: #00a884; }
.req-md-content ol li::marker { color: #8696a0; }
.req-md-content br + br { display: none; }
.req-placeholder { color: #8696a0; font-style: italic; }
...
.req-tag.manual { background: #00a88430; color: #00a884; }

/* ── 📄 客户单证面板 ── */
.doc-panel-body { padding: 0; height: 100%; overflow-y: auto; }
.doc-card-list { padding: 8px 12px; }
.doc-card { background: #202c33; border-radius: 8px; padding: 10px 12px; margin-bottom: 8px; border: 1px solid #2a3942; }
...
.doc-type-tag.dt-pi { background: #00a88430; color: #00a884; }
.doc-type-tag.dt-quotation { background: #7c3aed30; color: #a78bfa; }
.doc-number { color: #8696a0; font-size: 12px; font-family: monospace; flex: 1; }
.doc-status { font-size: 11px; padding: 1px 6px; border-radius: 4px; }
.doc-status.ds-draft { background: #3b4a54; color: #8696a0; }
.doc-status.ds-confirmed { background: #f59e0b25; color: #f59e0b; }
.doc-status.ds-sent { background: #00a88430; color: #00a884; }
...
.doc-amount { color: #e9edef; font-weight: 600; font-size: 15px; }
.doc-date { color: #8696a0; font-size: 11px; }
.doc-card-actions { display: flex; gap: 6px; margin-top: 6px; }
.doc-action-btn { background: transparent; border: 1px solid #3b4a54; color: #8696a0; padding: 4px 10px; border-radius: 4px; font-size: 12px; cursor: pointer; transition: all 0.15s; }
.doc-action-btn:hover { background: #2a3942; color: #e9edef; border-color: #00a884; }
...
.doc-type-card { background: #202c33; border-radius: 12px; padding: 20px; width: 320px; max-width: 100%; border: 1px solid #3b4a54; }
.doc-type-option { display: flex; flex-direction: column; gap: 4px; width: 100%; padding: 12px; background: #2a3942; border: 1px solid #3b4a54; border-radius: 8px; color: #e9edef; cursor: pointer; margin-bottom: 8px; align-items: flex-start; text-align: left; transition: all 0.15s; }
.doc-type-option:hover { background: #00a88430; border-color: #00a884; }
.doc-type-cancel { width: 100%; padding: 8px; background: transparent; color: #8696a0; border: none; cursor: pointer; font-size: 13px; margin-top: 4px; }
.doc-type-cancel:hover { color: #e9edef; }
...
.mdp-logo { color: #e9edef; font-size: 18px; font-weight: 600; }
.mdp-close {
  background: none; border: none; color: #8696a0;
...
.mdp-close:active { background: #2a3942; color: #e9edef; }
.mdp-wa-status {
  padding: 10px 18px;
  display: flex; align-items: center; gap: 8px;
  font-size: 12px; color: #8696a0;
...
.mdp-wa-status.connected { color: #00a884; }
.mdp-status-dot {
  width: 8px; height: 8px; border-radius: 50%;
  background: #8696a0;
}
.mdp-wa-status.connected .mdp-status-dot { background: #00a884; box-shadow: 0 0 6px #00a88460; }
.mdp-nav {
...
.mdp-item {
  ...
  color: #e9edef; font-size: 15px; text-align: left;
  cursor: pointer;
  -webkit-tap-highlight-color: transparent;
}
.mdp-item:active { background: #202c33; }
.mdp-item.active { color: #00a884; background: #00a88415; }
.mdp-icon {
  width: 28px; font-size: 20px; display: flex;
  align-items: center; justify-content: center;
  color: #8696a0;
}
.mdp-item.active .mdp-icon { color: #00a884; }
.mdp-label { flex: 1; }
.mdp-badge {
...
.mdp-footer {
  border-top: 1px solid #222d34; padding: 6px 0;
}
.mdp-logout-btn { color: #f15c6d; }
.mdp-logout-btn .mdp-icon { color: #f15c6d; }

/* ── 📄 单证快捷生成面板 v2 ── */
.doc-panel-v2 { display: flex; flex-direction: column; height: 100%; background:#111b21; position: relative; }
.doc-new-formal-btn {
  width: 30px; height: 30px; border-radius: 6px; border: none;
  background: #2a3942; color: #8696a0; cursor: pointer; font-size: 20px; line-height:1;
  display: flex; align-items: center; justify-content: center;
  margin-left: auto; transition: all .15s ease; font-family: inherit;
}
.doc-new-formal-btn:hover { background: #3b4a54; color: #e9edef; }
...
.doc-type-chip {
  ...
  background: #202c33; color: #d1d7db; border: 1.5px solid #2a3942; border-radius: 10px;
...
.doc-mode-btn {
  flex: 1; background: #202c33; color: #8696a0; border: 1px solid #2a3942;
...
.doc-result-scroll::-webkit-scrollbar-thumb { background: #2a3942; border-radius: 3px; }

.doc-empty { text-align: center; padding: 40px 12px 24px; }
.doc-empty-icon { font-size: 48px; margin-bottom: 10px; opacity: .7; }
.doc-empty-title { color: #e9edef; font-size: 14px; margin-bottom: 6px; font-weight: 500; }
.doc-empty-sub { color: #8696a0; font-size: 12px; line-height: 1.6; padding: 0 12px; }
...
.doc-result-head {
  ...
  padding: 10px 12px; border-bottom: 1px solid #2a3942;
  background: linear-gradient(135deg, #202c33, #1a262d);
}
.doc-result-type-tag {
  background: #3b4a54; color: #8696a0; font-size: 11px; font-weight: 600;
  padding: 3px 9px; border-radius: 10px; display: inline-flex; align-items: center; gap: 4px;
}
.doc-result-time { color: #8696a0; font-size: 11px; }
...
.doc-fade-mask {
  ...
  background: linear-gradient(to bottom, rgba(32,44,51,0), #202c33 80%);
  pointer-events: none;
}
.doc-expand-btn {
  width: 100%; background: transparent; border: none; border-top: 1px solid #2a3942;
  color: #c4b5fd; font-size: 12px; padding: 8px; cursor: pointer; font-family: inherit;
  transition: background .15s;
}
.doc-expand-btn:hover { background: #26353d; }

.doc-result-body { padding: 10px 12px; color: #d1d7db; font-size: 13px; line-height: 1.6; word-break: break-word; }
.doc-doc-title { color: #fff; font-size: 16px; font-weight: 700; text-align: center; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px dashed #2a3942; }
...
.doc-section-body { color: #d1d7db; font-size: 12.5px; line-height: 1.65; }
.doc-section-body table { width:100%; border-collapse:collapse; background:#111b21; border-radius:6px; overflow:hidden; margin:6px 0; font-size:12px; }
.doc-section-body th { background:#0f1920; color:#00a884; font-weight:600; padding:6px 7px; text-align:left; border-bottom:1px solid #2a3942; white-space:nowrap; font-size:11.5px; }
.doc-section-body td { padding:6px 7px; border-bottom:1px solid #1f2c33; color:#d1d7db; vertical-align:top; font-size:11.5px; }
.doc-section-body p { margin: 3px 0; }
.doc-section-body b,.doc-section-body strong { color:#e9edef; font-weight:700 }
.doc-section-body ul,.doc-section-body ol { padding-left:18px; margin:4px 0; }
.doc-section-body li { margin-bottom:2px; color:#d1d7db; font-size:12px; }

/* Tables (fallback for markdown-body outside sections) */
.markdown-body table { width:100%; border-collapse:collapse; background:#111b21; border-radius:6px; overflow:hidden; margin:8px 0; }
.markdown-body th { background:#1a262d; color:#00a884; font-weight:600; padding:7px 9px; text-align:left; border-bottom:1px solid #2a3942; font-size:12px; white-space:nowrap; }
.markdown-body td { padding:7px 9px; border-bottom:1px solid #1f2c33; color:#d1d7db; font-size:12px; vertical-align:top; }
.markdown-body h1 { color:#fff;font-size:17px;font-weight:700;margin:8px 0 12px;text-align:center }
.markdown-body h2 { color:#e9edef;font-size:15px;font-weight:700;margin:14px 0 8px }
.markdown-body h3 { color:#e9edef;font-size:13.5px;font-weight:700;margin:12px 0 6px;padding-bottom:4px;border-bottom:1px solid #2a3942 }
.markdown-body ul,.markdown-body ol { padding-left:20px;margin:6px 0 }
.markdown-body li { margin-bottom:3px;color:#d1d7db;font-size:13px }
.markdown-body b,.markdown-body strong { color:#e9edef;font-weight:700 }
.markdown-body pre { background:#111b21;border-radius:6px;padding:10px;overflow-x:auto;font-size:12px;color:#d1d7db;margin:6px 0;white-space:pre-wrap }
.markdown-body code { background:#111b21;padding:1px 5px;border-radius:3px;font-size:12px;color:#e9edef;font-family:ui-monospace,Menlo,Consolas,monospace }
.markdown-body hr { border:none;border-top:1px solid #2a3942;margin:10px 0 }
...
.doc-head-icon-btn {
  ...
  background: transparent; color: #8696a0; cursor: pointer; font-size: 13px;
  display: inline-flex; align-items: center; justify-content: center; transition: all .15s;
  font-family: inherit;
}
.doc-head-icon-btn:hover { background: #2a3942; color: #e9edef; }
.doc-saved-badge {
...
.doc-result-bottom-actions {
  display: flex; gap: 6px; padding: 10px; border-top: 1px solid #2a3942;
  background: #1a262d;
}
.doc-act-sec {
  flex: 1; background: #2a3942; color: #d1d7db; border: none; border-radius: 8px;
...
.doc-act-sec:hover { background: #3b4a54; color: #fff; }
...
.doc-typing-text { color: #8696a0; font-size: 12px; margin-top: 10px; }
...
.doc-ai-bubble {
  background: #202c33; color: #e9edef; border-radius: 10px; padding: 14px 18px;
  display: inline-flex; align-items: center; gap: 4px;
}
.doc-loading-bubble { display: inline-flex; align-items: center; gap: 4px; }
.doc-msg-time { font-size: 10px; color: #8696a0; margin-top: 3px; padding: 0 4px; }

/* ── v3 新增样式：紧凑条/语言/编辑/PDF ── */
.doc-compact-bar {
  display: flex; align-items: center; gap: 8px; padding: 8px 12px;
  border-bottom: 1px solid #222d34; background: #111b21; flex-shrink: 0;
}
.doc-current-chip {
  display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px;
  border-radius: 20px; background: #2a3942; color: #e9edef; font-size: 13px; font-weight: 600;
  border: 1.5px solid #7c3aed; white-space: nowrap;
}
.doc-chip-ic { font-size: 15px; }
.doc-compact-act {
  background: #202c33; border: 1px solid #3b4a54; color: #e9edef; font-size: 12px;
  padding: 6px 10px; border-radius: 16px; cursor: pointer; white-space: nowrap;
  transition: all .15s ease;
}
.doc-compact-act:hover { background: #2a3942; color: #00a884; border-color: #00a884; }
.doc-compact-act.open { border-color: #7c3aed; color: #c4b5fd; }
.doc-compact-act-danger { color: #f87171; border-color: #3b4a54; padding: 6px 10px; }
...
.doc-lang-picker {
  ...
  background: #202c33; border: 1px solid #3b4a54; border-radius: 10px;
  max-height: 260px; overflow-y: auto; box-shadow: 0 8px 24px rgba(0,0,0,.4);
  padding: 4px 0;
}
.doc-lang-option {
  display: flex; align-items: center; gap: 10px; padding: 10px 16px;
  color: #e9edef; font-size: 13.5px; cursor: pointer; transition: background .1s ease;
}
.doc-lang-option:hover { background: #2a3942; }
.doc-lang-option.active { background: #2a3942; color: #00a884; }
...
.doc-main-bottom { padding-top: 10px; border-top: 1px solid #222d34; }
.doc-lang-tag { color: #8696a0; font-weight: 400; font-size: 11px; }
...
.doc-edit-textarea {
  ...
  color: #e9edef; font-size: 13.5px; line-height: 1.6; resize: none; outline: none;
  font-family: 'SF Mono', Menlo, Consolas, 'PingFang SC', 'Microsoft YaHei', monospace;
  box-shadow: 0 0 0 3px rgba(124,58,237,.12);
}
.doc-edit-textarea::placeholder { color: #8696a0; }
.doc-edit-footer {
  display: flex; gap: 8px; padding-top: 6px;
}
.doc-edit-cancel {
  flex: 0 0 auto; padding: 10px 18px; background: #2a3942; color: #8696a0;
  border: 1px solid #3b4a54; border-radius: 8px; cursor: pointer; font-size: 13px;
}
.doc-edit-cancel:hover { color: #e9edef; }
.doc-edit-apply {
...
.doc-send-btn:disabled { background: #3b4a54; cursor: not-allowed; }
...
.doc-history-details { border-top: 1px solid #222d34; background: #0f1920; flex-shrink: 0; }
.doc-history-summary {
  padding: 9px 12px; color: #8696a0; font-size: 12.5px; cursor: pointer;
...
.doc-history-count {
  background: #2a3942; color: #8696a0; border-radius: 10px; padding: 1px 8px; font-size: 11px;
}
...
.doc-history-empty { color: #8696a0; font-size: 12px; text-align: center; padding: 10px 0; }
...
@media (max-width: 768px) {
  ...
  .workspace.has-mheader { padding-top: 0; }
  .workspace.has-tabbar { padding-bottom: 62px; }
}
...
.col-conversation.email-conv { background: #111b21; }
