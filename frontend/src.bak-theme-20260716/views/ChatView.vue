        ...
        <div class="pp-scroll" @click.self="closeProfile">
          ...
          <div class="pp-section">
            ...
            <template v-if="!profileEditing">
              ...
              <div class="pp-view-row" v-if="profileForm.source && profileForm.source!=='whatsapp'"><div class="pp-view-label">来源</div><div class="pp-view-val">{{ sourceLabel(profileForm.source) }}</div></div>
            ...
            <div class="pp-field">
              ...
              <select v-model="profileForm.source">
                ...
                <option value="website">独立站询盘</option>
                <option value="proactive">主动开发</option>
...
<!-- ⚡ 首响等待提示 -->
      <div v-if="activeFirstResponse" class="fr-wait-banner">
          ...
          <template v-for="(group, gIdx) in groupedMessages" :key="gIdx">
            ...
            <div v-for="msg in group.msgs" :key="msg.id" class="msg" :class="{ incoming: !msg.fromMe, outgoing: msg.fromMe }">
              <div class="msg-bubble">
                <!-- 悬浮 AI 按钮：仅在客户消息（incoming）上显示，默认隐藏，hover 显示 -->
                <button
                  v-if="!msg.fromMe"
                  class="msg-ai-btn"
                  title="AI 帮我回复"
                  @click="onMsgAIReply(msg)"
                >🤖</button>
                ...
                <template v-if="msg.messageType === 'video' || msg.type === 'video' || (msg.mimeType && msg.mimeType.startsWith('video/'))">
                  <div class="msg-media-video">
                    ...
                    <div v-else class="msg-video-placeholder">📹 {{ msg.fileName || msg.body || '视频' }}</div>
                  </div>
                ...
                <template v-else-if="msg.messageType === 'document' || msg.type === 'document'">
                  ...
                  >
                    ...
                    <a v-if="!isPdfMsg(msg)" class="mmd-download-hidden" :href="'/api/wa/media?url=' + encodeURIComponent(msg.mediaUrl)" :download="msg.fileName || 'file'" target="_blank"></a>
                  </div>
      ...
      <div class="conv-input-area" v-if="chatStore.isConnected && chatStore.activeConversation">
        ...
        <button v-if="!pendingFile" class="send-btn" @click="onSendClick"><svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg></button>
      </div>
      ...
      <!-- PDF 全屏预览层 -->
      <div v-if="pdfPreviewUrl" class="pdf-preview-overlay" @click.self="closePdfPreview">
...
<script setup>
import { ref, computed, onMounted, onUnmounted, onBeforeUnmount, nextTick, watch, reactive } from 'vue';
import { useChatStore } from '../stores/chat.js';
import { useSocket } from '../utils/socket.js';
import { ElMessage, ElMessageBox } from 'element-plus';

const emit = defineEmits(['go-back','open-ai-panel','open-translate-panel']);
...
function onMediaDocClick(msg) {
  ...
  if (isPdfMsg(msg)) {
    const url = '/api/wa/media?url=' + encodeURIComponent(msg.mediaUrl) + '&inline=1';
    pdfPreviewUrl.value = url;
  } else {
    // trigger download for non-PDF: same as <a download>
    const a = document.createElement('a');
    a.href = '/api/wa/media?url=' + encodeURIComponent(msg.mediaUrl);
...
function closePdfPreview() { pdfPreviewUrl.value = ''; }
...
function sourceLabel(s) {
  return ({whatsapp:'WhatsApp',exhibition:'展会',website:'独立站询盘',proactive:'主动开发',referral:'老客户推荐',email:'邮件营销',other:'其他'})[s] || s;
}
...
async function saveProfile() {
  ...
  try {
    ...
    payload.tags = tagsArr.length ? JSON.stringify(tagsArr) : '[]';
    delete payload.tagsInput;
    delete payload.tagsDisplay;
    const api = await getApi();
    ...
    if (data.name) {
      const conv = chatStore.conversations.find(c => c.jid === chatStore.activeJid.value);
      if (conv) conv.name = data.name;
      chatStore.activeConversation && (chatStore.activeConversation.name = data.name);
    }
...
// 点击消息旁小圆点：触发 AI 回复并展开 AI 面板
function onMsgAIReply(msg) {
  chatStore.triggerAIReplyForMessage(msg);
  emit('open-ai-panel');
}
...
<style scoped>
.conv-page { display: flex; flex-direction: column; height: 100%; background: #0b141a; position: relative; }

/* ── 未连接/扫码/无会话 等页面样式（保持原样） ── */
.wa-intro { flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:40px 32px; color:#e9edef; background:#111b21; border-bottom:6px solid #00a884; overflow-y:auto; }
.wa-intro-art { margin-bottom:28px; opacity:.9; }
.wa-intro-svg { max-width:303px; width:100%; height:auto; }
.wa-intro-title { font-size:28px; font-weight:300; color:#e9edef; margin:0 0 14px; }
.wa-intro-sub { font-size:14px; color:#8696a0; text-align:center; max-width:440px; line-height:1.5; margin:0 0 32px; }
.wa-login-btn { display:inline-flex; align-items:center; gap:10px; padding:12px 28px; background:#00a884; color:#111b21; border:none; border-radius:24px; font-size:14px; font-weight:600; cursor:pointer; transition:background .15s; font-family:inherit; }
.wa-login-btn:hover:not(:disabled) { background:#06cf9c; }
.wa-login-btn:disabled { opacity:.7; cursor:wait; }
.wa-spin { animation: wa-spin 1s linear infinite; }
@keyframes wa-spin { to { transform: rotate(360deg); } }
.wa-intro-tips { margin-top:40px; color:#8696a0; font-size:12px; text-align:center; }
.wa-intro-tips p { margin:6px 0; display:flex; align-items:center; justify-content:center; gap:6px; }
.wa-lock { font-size:14px; }

.wa-scan-wrap { flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#111b21; padding:24px; position:relative; }
.wa-scan-back { position:absolute; top:16px; left:16px; width:36px; height:36px; background:#202c33; border:none; color:#e9edef; border-radius:50%; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:background .15s; }
.wa-scan-back:hover { background:#2a3942; }
.wa-scan-card { display:flex; gap:40px; background:#111b21; border-radius:4px; padding:48px 56px 40px; max-width:820px; width:100%; align-items:flex-start; }
.wa-scan-left { flex:1; min-width:0; color:#e9edef; }
.wa-scan-right { flex-shrink:0; display:flex; flex-direction:column; align-items:center; }
.wa-scan-title { font-size:28px; font-weight:300; color:#e9edef; margin:0 0 32px; line-height:1.2; }
.wa-scan-steps { list-style:none; padding:0; margin:0 0 32px; }
.wa-scan-steps li { display:flex; align-items:flex-start; gap:18px; margin-bottom:20px; font-size:14px; line-height:1.5; color:#d1d7db; }
.step-num { flex-shrink:0; width:24px; height:24px; border-radius:50%; background:transparent; border:2px solid #8696a0; color:#8696a0; display:flex; align-items:center; justify-content:center; font-size:12px; font-weight:600; }
.step-text strong { color:#e9edef; font-weight:600; }
.wa-help-link { display:inline-block; color:#53bdeb; text-decoration:none; font-size:14px; margin-bottom:28px; }
.wa-help-link:hover { text-decoration:underline; }
.wa-stay-logged { display:inline-flex; align-items:center; gap:10px; font-size:14px; color:#d1d7db; cursor:pointer; margin-bottom:24px; user-select:none; }
.wa-stay-logged input { display:none; }
.checkbox-box { width:18px; height:18px; border-radius:3px; border:2px solid #8696a0; display:flex; align-items:center; justify-content:center; flex-shrink:0; transition:all .15s; }
.checkbox-box.checked { background:#00a884; border-color:#00a884; }
.wa-phone-login { display:inline-flex; align-items:center; gap:6px; color:#53bdeb; font-size:14px; cursor:pointer; margin-top:4px; }
...
.corner { position:absolute; width:24px; height:24px; border:4px solid #00a884; }
...
.qr-hint-text { margin-top:16px; font-size:12px; color:#8696a0; text-align:center; }
.qr-hint-text strong { color:#00a884; }
...
.qr-loading-spin { width:36px; height:36px; border:3px solid #e9edef; border-top-color:#00a884; border-radius:50%; animation: wa-spin .8s linear infinite; }
.qr-loading-text { font-size:13px; color:#667781; }
.qr-expired-title { font-size:16px; font-weight:500; color:#111b21; }
.qr-error-icon { width:40px; height:40px; border-radius:50%; background:#ea4335; color:#fff; display:flex; align-items:center; justify-content:center; font-size:22px; font-weight:700; }
.qr-error-text { font-size:13px; color:#111b21; line-height:1.4; }
.qr-reload-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 18px; background:#00a884; color:#fff; border:none; border-radius:20px; font-size:13px; font-weight:500; cursor:pointer; transition:background .15s; font-family:inherit; }
.qr-reload-btn:hover { background:#06cf9c; }
.wa-scan-footer { margin-top:24px; text-align:center; color:#8696a0; font-size:12px; }
.wa-scan-footer p { margin:6px 0; }
.wa-scan-lock { display:flex; align-items:center; justify-content:center; }
.wa-scan-footer a { color:#53bdeb; text-decoration:none; }
.wa-scan-footer a:hover { text-decoration:underline; }
.wa-scan-terms .dot { margin:0 6px; }
.wa-scan-create a { color:#00a884; }

.wa-noselect { flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:40px 32px; background:#111b21; border-bottom:6px solid #00a884; color:#e9edef; }
.wa-noselect-art { margin-bottom:28px; opacity:.75; }
.wa-noselect-title { font-size:28px; font-weight:300; margin:0 0 12px; color:#e9edef; }
.wa-noselect-sub { font-size:13px; color:#8696a0; text-align:center; max-width:440px; line-height:1.5; margin:0; }
.wa-noselect-divider { width:240px; height:1px; background:#222d34; margin:32px 0 24px; }
.wa-noselect-tip { font-size:14px; color:#d1d7db; margin:0 0 16px; }
.wa-noselect-encrypt { font-size:12px; color:#8696a0; margin:0; }

/* ── ⚡ 首响等待提示 banner ── */
.fr-wait-banner {
  background: linear-gradient(90deg, #f15c6d, #b9303f);
  color: #fff;
  padding: 8px 16px;
  font-size: 13px;
  display: flex;
...
@keyframes frb-pulse-chat {
  0%, 100% { background: linear-gradient(90deg, #f15c6d, #b9303f); }
...
/* ── 对话 Header ── */
.conv-header { height:56px; background:#202c33; border-bottom:1px solid #222d34; display:flex; align-items:center; padding:0 16px; gap:12px; flex-shrink:0; }
.back-btn { display:none; background:none; border:none; color:#e9edef; cursor:pointer; padding:6px; }
...
.ch-status { font-size:12px; color:#8696a0; display:flex; align-items:center; gap:4px; margin-top:1px; }
.online-dot { width:7px; height:7px; border-radius:50%; background:#00a884; }
.online-dot.offline { background:#667781; }
.ch-actions { display:flex; gap:6px; }
.ch-actions button { width:34px; height:34px; display:flex; align-items:center; justify-content:center; background:none; border:none; color:#8696a0; border-radius:50%; cursor:pointer; }
...
.msgs-loading { display:flex; justify-content:center; padding:20px; color:#8696a0; font-size:12px; }
.loading-dots::after { content:''; animation: dots 1.5s infinite; }
@keyframes dots { 0%,20%{content:'.'} 40%{content:'..'} 60%,100%{content:'...'} }
.msg-date-divider { display:flex; justify-content:center; margin:12px 0; }
.msg-date-divider span { background:#202c33; color:#8696a0; padding:5px 12px; border-radius:8px; font-size:11px; }
...
.msg-bubble {
  max-width:75%; padding:8px 12px 6px; border-radius:8px;
  font-size:14.5px; line-height:1.4; position:relative; word-wrap:break-word;
  display:flex; flex-direction:column; gap:0;
}
.msg.incoming .msg-bubble { background:#202c33; color:#e9edef; border-top-left-radius:2px; }
.msg.outgoing .msg-bubble { background:#005c4b; color:#e9edef; border-top-right-radius:2px; }
...
.msg-retranslate-btn:hover { opacity:1; color:#53bdeb; }
...
.pdf-preview-frame {
  width:92vw; height:92vh; border:none; border-radius:4px; background:#fff;
...
.msg-time {
  align-self:flex-end; font-size:10.5px; color:#8696a0;
  display:flex; align-items:center; gap:3px; margin-top:4px;
}
.msg-check { color:#53bdeb; font-size:12px; margin-left:3px; }
.msg-check.pending { color:#8696a0; font-size:11px; }
.msg-check.sent { color:#8696a0; font-size:12px; }
.msg-check.delivered { color:#8696a0; font-size:12px; }
.msg-check.read { color:#53bdeb; font-size:12px; }
.msg-check.error { color:#f15c6d; font-size:11px; }
...
.msg-ai-btn {
  ...
  opacity:0;
  transform:scale(.8);
  transition: opacity .15s ease, transform .15s ease, background .15s, color .15s;
  z-index:5;
  box-shadow:0 1px 3px rgba(0,0,0,.3);
}
.msg.incoming:hover .msg-ai-btn,
.msg-ai-btn:focus-visible {
  opacity:1;
  transform:scale(1);
}
.msg-ai-btn:hover {
...
/* ── 输入区 ── */
.conv-input-area { background:#202c33; padding:8px 12px; display:flex; align-items:center; gap:8px; flex-shrink:0; }
.input-action, .emoji-btn { background:none; border:none; color:#8696a0; cursor:pointer; padding:6px; border-radius:50%; display:flex; align-items:center; justify-content:center; }
.input-action:hover, .emoji-btn:hover { color:#e9edef; }
.input-wrap { flex:1; background:#2a3942; border-radius:22px; display:flex; align-items:center; padding:0 6px 0 16px; }
.input-wrap input { flex:1; background:transparent; border:none; color:#e9edef; padding:10px 0; font-size:14px; outline:none; font-family:inherit; }
.send-btn { width:40px; height:40px; border-radius:50%; background:#00a884; color:#fff; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:background .15s; }
.send-btn:hover { background:#06cf9c; }
/* ── 内嵌翻译确认面板 ── */
.trans-confirm-panel { position:relative; margin:0 80px 8px 16px; background:#202c33; border-radius:8px; box-shadow:0 2px 10px rgba(0,0,0,.4); padding:12px 14px; border:1px solid #2a3942; }
.tcp-row { display:flex; align-items:flex-start; gap:8px; }
.tcp-top { justify-content:space-between; align-items:center; }
.tcp-label { font-size:13px; color:#8696a0; flex-shrink:0; line-height:1.5; }
.tcp-text { flex:1; font-size:14px; line-height:1.5; word-break:break-all; }
.tcp-translated { color:#e9edef; font-weight:500; }
.tcp-original { color:#8696a0; }
.tcp-error { color:#ea6b6b !important; }
.tcp-swap { display:inline-flex; align-items:center; background:none; border:1px solid #3b4a54; color:#00a884; font-size:12px; cursor:pointer; padding:3px 8px; border-radius:4px; flex-shrink:0; font-family:inherit; transition:all .15s; }
.tcp-swap:hover { background:#00a88420; border-color:#00a884; color:#00a884; }
.tcp-divider { height:1px; border-top:1px dashed #3b4a54; margin:8px 0; }
.tcp-tips { margin-top:8px; text-align:right; font-size:11px; color:#8696a0; line-height:1.5; }
...
@media (max-width: 768px) {
  ...
  .msg.incoming { padding-right:36px; }
  .msg-ai-btn { right:-32px; width:26px; height:26px; font-size:13px; opacity:1; transform:scale(1); }
...
.mpb-name { font-size:13px; color:#e9edef; max-width:160px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.mpb-size { font-size:11px; color:#8696a0; margin-top:2px; }
.mpb-caption {
  flex:1;
  background:#2a3942; border:none; outline:none;
  color:#e9edef; padding:8px 12px; border-radius:18px;
...
.mpb-send:disabled { background:#3b4a54; cursor:wait; }
.mpb-cancel {
  width:32px; height:32px; border-radius:50%;
  background:none; border:none; color:#8696a0; cursor:pointer;
  display:flex; align-items:center; justify-content:center; flex-shrink:0;
}
.mpb-cancel:hover { color:#e9edef; }
.mpb-spin {
...
.msg-video-placeholder { padding:28px 18px; color:#fff; text-align:center; font-size:14px; background:#202c33; border-radius:8px; }
.msg-media-doc {
  display:flex; align-items:center; gap:10px;
...
.msg.incoming .mmd-icon { background:#2a3942; }
.mmd-info { min-width:0; }
.mmd-name { font-size:14px; color:#e9edef; word-break:break-all; }
.mmd-size { font-size:11px; color:#8696a0; margin-top:2px; }
.msg-caption { font-size:13px; color:#e9edef; margin-top:4px; padding-right:0; white-space:pre-wrap; line-height:1.4; }
@media (max-width:768px) {
...
@media (max-width: 768px) {
  ...
  .aisb-foreign {
    font-size: 13px; line-height: 1.35; color: #e9edef;
    display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;
    overflow: hidden; word-break: break-word;
  }
  .aisb-zh {
    font-size: 11px; line-height: 1.3; color: #8696a0; font-style: italic;
    margin-top: 3px;
  ...
  .aisb-refresh:hover { color: #00a884; background: #202c33; }
  ...
  .aisb-skeleton {
    ...
    height: 30px;
    border-radius: 20px;
    background: linear-gradient(90deg, #202c33 0%, #2a3942 50%, #202c33 100%);
...
.profile-panel{width:100%;max-width:400px;background:#111b21;display:flex;flex-direction:column;height:100%;box-shadow:-4px 0 20px rgba(0,0,0,.4);transform:translateX(0)}
.pp-header{height:56px;background:#202c33;display:flex;align-items:center;padding:0 8px;flex-shrink:0;gap:8px}
.pp-back{background:transparent;border:none;color:#aebac1;cursor:pointer;padding:8px;border-radius:50%;display:flex;align-items:center;justify-content:center}
.pp-back:hover{background:#2a3942}
.pp-title{flex:1;color:#e9edef;font-size:16px;font-weight:500;text-align:center}
.pp-scroll{flex:1;overflow-y:auto;background:#111b21}
.pp-avatar-area{padding:28px 16px 20px;display:flex;flex-direction:column;align-items:center;background:#111b21}
.pp-avatar{width:140px;height:140px;border-radius:50%;background:#00a884;color:#fff;display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;cursor:pointer}
.pp-avatar-img{width:100%;height:100%;object-fit:cover}
.pp-name{margin-top:16px;font-size:22px;color:#e9edef;font-weight:500;text-align:center;word-break:break-word}
.pp-phone{margin-top:6px;color:#00a884;font-size:14px}
...
.pp-level-tag.lv-A{background:#00a884;color:#fff}
.pp-level-tag.lv-B{background:#53bdeb;color:#111b21}
.pp-level-tag.lv-C{background:#3b4a54;color:#aebac1}
.pp-level-tag.lv-D{background:#f15c6d;color:#fff}
.pp-status-tag{font-size:12px;padding:3px 10px;border-radius:10px;background:#2a3942;color:#aebac1}
.pp-quick{display:flex;background:#111b21;border-bottom:8px solid #0b141a;padding:0 16px 16px;gap:8px}
.pp-quick-btn{flex:1;background:#202c33;border:none;color:#aebac1;cursor:pointer;padding:12px 4px;border-radius:8px;display:flex;flex-direction:column;align-items:center;gap:6px;font-size:12px;transition:background .15s}
.pp-quick-btn:hover{background:#2a3942}
.pp-tabs{display:flex;background:#111b21;border-bottom:1px solid #222d34}
.pp-tab{flex:1;text-align:center;padding:12px 0;color:#8696a0;font-size:14px;cursor:pointer;border-bottom:3px solid transparent;transition:color .15s}
.pp-tab.active{color:#00a884;border-bottom-color:#00a884}
.pp-empty-tab{padding:32px 16px;text-align:center;color:#3b4a54;font-size:13px;background:#111b21;border-bottom:8px solid #0b141a}
.pp-section{background:#111b21;padding:0 16px;border-bottom:8px solid #0b141a}
.pp-section-title{color:#00a884;font-size:13px;font-weight:600;padding:16px 0 8px;text-transform:uppercase;letter-spacing:.3px}
.pp-field{padding:10px 0;border-bottom:1px solid #222d34}
.pp-field-tall{padding:14px 0}
.pp-field label{display:block;color:#8696a0;font-size:12px;margin-bottom:4px}
.pp-field input,.pp-field select,.pp-field textarea{width:100%;background:transparent;border:none;color:#e9edef;font-size:15px;padding:6px 0;outline:none;font-family:inherit;resize:vertical}
...
.pp-field input::placeholder,.pp-field textarea::placeholder{color:#3b4a54}
.pp-field select option{background:#202c33;color:#e9edef}
.pp-save-btn{width:100%;background:#00a884;color:#111b21;border:none;padding:12px;border-radius:8px;font-size:15px;font-weight:600;cursor:pointer;margin:16px 0;transition:background .15s}
.pp-save-btn:hover{background:#06cf9c}
.pp-save-btn:disabled{opacity:.5;cursor:not-allowed}
.pp-switch-row{display:flex;align-items:center;justify-content:space-between;padding:14px 0;border-bottom:1px solid #222d34}
.pp-switch-row:last-child{border-bottom:none}
.pp-switch-label{}
.pp-switch-label>div:first-child{color:#e9edef;font-size:15px}
.pp-switch-sub{color:#8696a0;font-size:13px;margin-top:2px}
...
.pp-slider{position:absolute;cursor:pointer;top:0;left:0;right:0;bottom:0;background:#3b4a54;border-radius:26px;transition:.2s}
...
.pp-switch input:checked+.pp-slider{background:#00a884}
.pp-switch input:checked+.pp-slider:before{transform:translateX(20px)}
.pp-encryption{padding:16px;text-align:center;color:#8696a0;font-size:12px}
.pp-bottom-spacer{height:20px}
.pp-edit-btn{background:transparent;border:none;color:#aebac1;cursor:pointer;padding:8px;border-radius:50%;display:flex;align-items:center;justify-content:center}
.pp-edit-btn:hover{background:#2a3942;color:#e9edef}
.pp-view-row{display:flex;justify-content:space-between;align-items:flex-start;gap:16px;padding:12px 0;border-bottom:1px solid #222d34}
.pp-view-row:last-child{border-bottom:none}
.pp-view-label{color:#8696a0;font-size:13px;flex-shrink:0;min-width:80px}
.pp-view-val{color:#e9edef;font-size:14px;text-align:right;word-break:break-word;flex:1}
.pp-view-empty{padding:32px 0;text-align:center;color:#3b4a54;font-size:14px}
.pp-tag-chip{display:inline-block;background:#202c33;color:#00a884;font-size:12px;padding:2px 10px;border-radius:10px;margin:2px 0 2px 4px}
/* 抽屉动画 */
