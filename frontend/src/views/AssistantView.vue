    ...
    <div class="input-area">
      ...
      <div class="input-card">
        ...
        <div v-else class="voice-placeholder">
          ...
          <span v-else class="voice-hint">按住说话</span>
        </div>
        <!-- 下半：工具栏 -->
        <div class="toolbar-row">
          <div class="toolbar-left">
            ...
            <button v-if="!isVoiceMode" class="tb-btn model-btn" @click.stop="showModelSheet = true" title="模型">
          ...
          <div class="toolbar-right">
            <button v-if="!isVoiceMode" class="tb-btn mic-btn" @click="toggleVoiceMode" title="语音输入">
            ...
            <button v-else class="tb-btn kb-btn" @click="toggleVoiceMode" title="切换键盘">
            ...
            <button v-else class="send-btn voice-send-btn" :class="{ recording: isRecording }" @click="isRecording ? stopVoice() : startVoice()" :disabled="loading">
              <svg v-if="!isRecording" viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm-1-9c0-.55.45-1 1-1s1 .45 1 1v6c0 .55-.45 1-1 1s-1-.45-1-1V5z"/></svg>
              <svg v-else viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2"/></svg>
            </button>
          </div>
        </div>
      </div>
...
const availableSkills = [
  ...
  // 客户管理
  { key: 'customer_research', label: '客户背调', icon: '🔍', desc: '全球企业深度背调' },
  ...
  // 物流运输
  { key: 'freight_forwarder', label: '货代背调', icon: '🚛', desc: '货代资质审核与评估' },
...
.model-btn:hover { background: var(--sidebar-active); opacity: 0.8; }
.model-label {
  font-size: 13px;
  font-weight: 500;
  color: var(--text-primary);
