  ...
  <div class="chat-window" @click="closeContextMenu">
    ...
    <div class="chat-header">
      <div class="header-info">
        <div class="header-avatar" :style="!headerAvatarOk ? {background: headerAvatarBg} : {}">
          <img v-if="chatStore.loadAvatar(jid) && headerAvatarOk" class="header-avatar-img" :src="chatStore.loadAvatar(jid)" @error="onHeaderAvatarError" alt="" />
          <span v-else class="avatar-text">{{ getInitial(contact?.name || contact?.phone || jid) }}</span>
        </div>
...
function avatarColor(name) {
  ...
  const palette = ['#00a884','#06cf9c','#7c3aed','#3b82f6','#ef4444','#f59e0b','#ec4899','#14b8a6','#f97316','#8b5cf6'];
  return palette[h % palette.length];
}
const headerAvatarBg = computed(() => avatarColor(props.contact?.name || props.contact?.phone || props.jid || ''));
