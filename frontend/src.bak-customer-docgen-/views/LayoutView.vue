  ...
  <div class="crm-layout" :class="{ 'platform-collapsed': platformCollapsed, 'mobile-view': isMobile }">
    ...
    <header class="mobile-header" v-if="isMobile && activePlatform === 'communication'">
      ...
      <div class="mh-title">
        <template v-if="mobileInConv">
          {{ chatStore.activeConversation?.name || '对话' }}
        </template>
        <template v-else>{{ currentChannel.name }}</template>
      </div>
      <div class="mh-right" v-if="mobileInConv">
        <button class="mh-btn" :class="{active: mobilePanel==='aitalk'}" @click="openMobilePanel('aitalk')" title="AI话术">💬</button>
        <button class="mh-btn" :class="{active: mobilePanel==='translate'}" @click="openMobilePanel('translate')" title="翻译">🌐</button>
        <button class="mh-btn" :class="{active: mobilePanel==='customer'}" @click="openMobilePanel('customer')" title="客户资料">👤</button>
      </div>
      <button class="mh-btn" v-if="!mobileInConv" @click="showSettings = true">⚙️</button>
    </header>
    ...
    <div class="workspace" :class="{ 'has-mheader': isMobile && activePlatform === 'communication' }">
      ...
      <div v-if="activePlatform === 'communication'" class="comm-module">
            ...
            <div class="wa-account-list">
              ...
              <div v-else class="wa-account-item active">
                <div class="acc-avatar">
                  {{ (chatStore.connectedPhone || 'W')[0] }}
                  <span class="acc-online-dot"></span>
                </div>
                <div class="acc-info">
                  <div class="acc-name">{{ chatStore.connectedPhone || 'WhatsApp' }}</div>
                  <div class="acc-meta online-text">在线</div>
                </div>
              </div>
              <!-- 其他渠道占位（本次只做 WhatsApp，邮箱不处理） -->
              <div v-if="activeChannel === 'email'" class="wa-connect-card">
                <div class="wa-connect-icon">📧</div>
                <div class="wa-connect-title">邮箱</div>
                <div class="wa-connect-desc">邮箱渠道功能开发中</div>
              </div>
            </div>
          </template>
        ...
        <aside class="col-chatlist" :class="{ collapsed: !isMobile && chatlistCollapsed, 'm-hidden': isMobile && mobileInConv }">
          ...
          <div class="chatlist-body" v-if="!chatlistCollapsed">
            ...
            <div
              ...
              :key="conv.jid"
              class="conv-item"
              :class="{ active: activeConv === conv.jid }"
            ...
            >
              ...
              <div class="conv-main">
                ...
                <div class="conv-row">
                  <span class="conv-preview">{{ conv.preview || '点击开始对话' }}</span>
        ...
        <main class="col-conversation" :class="{ 'm-in': isMobile && mobileInConv && activeConv }">
        ...
        <aside class="col-function-panel" :class="{ collapsed: !isMobile && panelCollapsed, mini: !isMobile && aiMiniMode && activePanel === 'ai', translating: !isMobile && activePanel === 'translate', aitalking: !isMobile && activePanel === 'aitalk', customering: !isMobile && activePanel === 'customer', 'm-panel': isMobile, 'm-panel-show': isMobile && mobilePanel }" :style="!isMobile && !panelCollapsed ? 'width:' + panelWidth + 'px' : ''">
          ...
          <div class="col-header panel-col-header" :class="{ 'ai-col-header': activePanel === 'ai' }">
            <template v-if="!panelCollapsed">
              ...
              <div class="ai-title" v-else-if="activePanel === 'customer'">
                <span class="ai-title-icon">👤</span>
                <span class="ai-title-text">客户资料</span>
            ...
            <button v-else class="col-toggle" @click="switchPanel(lastActivePanel || 'aitalk')" :title="lastActivePanel === 'translate' ? '翻译设置' : (lastActivePanel === 'customer' ? '客户资料' : (lastActivePanel === 'aitalk' ? 'AI话术' : 'AI助手'))">
              <span style="font-size:18px">{{ lastActivePanel === 'translate' ? '🌐' : (lastActivePanel === 'customer' ? '👤' : (lastActivePanel === 'aitalk' ? '💬' : '🤖')) }}</span>
            </button>
          </div>
          <!-- AI 助手面板 -->
          ...
          <div v-if="!panelCollapsed && activePanel === 'aitalk'" class="aitalk-panel-body">
            ...
            <div class="aitalk-top-bar">
              <div class="aitalk-top-first-row">
                <el-select v-model="aitalkModel" class="sub-select aitalk-model-select aitalk-top-first-select" popper-class="crm-dark-popper" placeholder="选择模型">
            ...
            <div ref="aitalkScrollRef" class="aitalk-scroll" @scroll="onAitalkScroll">
              ...
              <div v-if="aiMessages.length === 0 && !aitalkLoading" class="aitalk-welcome">
                ...
                <div class="aitalk-welcome-title">AI助手，帮你搞定外贸客户沟通</div>
                <div class="aitalk-chips">
                  <button class="aitalk-chip" @click="triggerQuickGenerate">✉️ 生成回复</button>
                  ...
                  <button class="aitalk-chip" @click="sendQuickPrompt('基于聊天记录推荐下一步跟进策略')">📋 推荐跟进策略</button>
                </div>
              </div>
              ...
              <template v-for="(msg, mi) in aiMessages" :key="msg.id">
                ...
                <!-- AI 回复 -->
                <div v-else class="aitalk-msg aitalk-msg-ai">
                  ...
                  <template v-else-if="msg.type === 'replies' && Array.isArray(msg.content)">
                    <div v-for="(r, idx) in msg.content" :key="idx" class="aitalk-reply-card-v2">
                      ...
                      <div class="aitalk-reply-actions">
                        <button class="aitalk-mini-btn" @click="copyForeign(r.foreign, $event)">📋 复制</button>
                      </div>
                      <button class="aitalk-use-btn" :class="{done: r._applied}" @click="useForeignReply(r, $event)">
                        {{ r._applied ? '✓ 已填入输入框' : '✏️ 使用这条回复' }}
                      </button>
                    </div>
                  </template>
            ...
            <div class="aitalk-footer">
              <template v-if="aitalkMode === 'quick'">
                ...
                >
                  ...
                  <span v-else>✨</span>
                  <span>{{ aitalkLoading ? 'AI分析中...' : (aiMessages.length ? '✨ 重新生成' : '✨ 生成AI回复') }}</span>
                </button>
          ...
          <div v-if="!panelCollapsed && activePanel === 'customer'" class="customer-panel-body">
            ...
            <template v-else>
              ...
              <!-- 警告条：无公司名 -->
              <div v-if="!customerEditMode && !customerData.companyName" class="customer-alert">
                <div class="customer-alert-text">
                  <span style="margin-right:6px">⚠️</span>还没获取到客户公司名，无法启动背调
                </div>
                <button class="customer-alert-btn" @click="generateCompanyAsk" :disabled="customerAskLoading">
                  {{ customerAskLoading ? '生成中…' : '🤖 生成询问话术' }}
                </button>
              </div>

              <!-- 询问话术卡片（生成后显示） -->
              <div v-if="customerAskReplies.length" class="customer-ask-list">
                <div v-for="(r, idx) in customerAskReplies" :key="idx" class="customer-ask-card">
                  <div class="customer-ask-name">{{ r.name }}</div>
                  <div class="customer-ask-foreign">{{ r.foreign }}</div>
                  <div class="customer-ask-chinese">{{ r.chinese }}</div>
                  <button class="customer-ask-use" @click="useAskReply(r.foreign)">✏️ 使用这条回复</button>
                </div>
              </div>

              <!-- 开始背调按钮（有公司名） -->
              <div v-if="!customerEditMode && customerData.companyName" class="customer-bg-btn-wrap">
                <button class="customer-bg-btn" @click="bgCheckComingSoon">🚀 开始背调</button>
              </div>
...
async function loadAiModels() {
  try {
    ...
    aiModels.value = list;
    if (list.length && !list.find(m => m.key === aitalkModel.value)) {
      const lite = list.find(m => (m.name || '').toLowerCase().includes('lite') || (m.name || '').includes('快速'));
      aitalkModel.value = (lite || list[0]).key;
    }
  } catch (e) {
    aiModels.value = [];
  }
...
async function triggerQuickGenerate() {
  ...
  aitalkLastRequest = { type: 'analyze', jid: chatStore.activeJid, model };
  try {
    const { data } = await api.post('/ai/analyze', { jid: chatStore.activeJid, accountId: 1, model, targetLang: resolveTargetLang() });
    if (data?.success && Array.isArray(data.data?.replies) && data.data.replies.length) {
      replaceMsg(loadingMsg.id, { role: 'assistant', type: 'replies', content: data.data.replies, ts: Date.now() });
    } else {
      replaceMsg(loadingMsg.id, { role: 'assistant', type: 'text', content: data?.error || '未生成回复，请重试', error: true, ts: Date.now() });
    }
  } catch (err) {
...
function bgCheckComingSoon() { ElMessage.info('背调功能开发中，敬请期待'); }
...
const platformItems = [
  ...
  { key: 'communication', label: '沟通', icon: '<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"/></svg>' },
...
const mobileTabs = [
  { key: 'chatlist', label: '聊天', icon: '<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"/></svg>' },
  { key: 'aitalk', label: 'AI话术', icon: '💬' },
...
function handleMobileTab(tab) {
  ...
  showSettings.value = false;
  if (tab.key === 'chatlist') {
    // 回到聊天列表：如果在对话中先退出对话；如果在其他平台切回沟通
    mobileInConv.value = false;
    mobilePanel.value = null;
  ...
  if (tab.key === 'aitalk' || tab.key === 'customer' || tab.key === 'translate') {
    // 若不在沟通模块或未选会话，提示
    if (activePlatform.value !== 'communication') {
      activePlatform.value = 'communication';
      if (route.path !== '/chat') router.push('/chat');
    }
    openMobilePanel(tab.key);
    return;
  }
  switchPlatform(tab);
}
...
const conversations = computed(() => {
  return chatStore.sortedConversations.map(c => ({
    jid: c.jid,
    id: c.jid,
...
.module-full { flex: 1; display: flex; flex-direction: column; overflow-y: auto; overflow-x: hidden; min-height: 0; -webkit-overflow-scrolling: touch; }
...
.trans-panel-body .el-select .el-select__wrapper,
.trans-panel-body .el-select .el-input__wrapper {
  background-color: transparent !important;
  box-shadow: 0 1px 0 0 #3b4a54 inset !important;
  border-radius: 0;
}
.trans-panel-body .el-select .el-select__wrapper:hover,
.trans-panel-body .el-select .el-input__wrapper:hover {
  box-shadow: 0 1px 0 0 #8696a0 inset !important;
}
.trans-panel-body .el-select .el-select__wrapper.is-focused,
.trans-panel-body .el-select .el-input__wrapper.is-focus {
  box-shadow: 0 2px 0 0 #00a884 inset !important;
}
...
.aitalk-model-select .el-select__wrapper { background: transparent !important; box-shadow: 0 1px 0 0 #3b4a54 inset !important; border-radius: 0; }
.aitalk-model-select .el-select__wrapper:hover { box-shadow: 0 1px 0 0 #8696a0 inset !important; }
.aitalk-model-select .el-select__wrapper.is-focused { box-shadow: 0 2px 0 0 #00a884 inset !important; }
...
.aitalk-lang-select .el-select__wrapper { background: transparent !important; box-shadow: 0 1px 0 0 #3b4a54 inset !important; border-radius: 0; }
.aitalk-lang-select .el-select__wrapper:hover { box-shadow: 0 1px 0 0 #8696a0 inset !important; }
.aitalk-lang-select .el-select__wrapper.is-focused { box-shadow: 0 2px 0 0 #00a884 inset !important; }
...
@media (max-width: 768px) {
  ...
  .workspace { flex: 1; min-height: 0; position: relative; overflow: hidden; height: 0; }
  .workspace.has-mheader { padding-bottom: 0; }

  /* 账号栏：抽屉宽度稍窄些，避免遮住整个屏幕 */
  .col-accounts { width: 78vw !important; max-width: 300px !important; z-index: 45; }
  ...
  /* 底部tabbar：适配 */
  .mobile-tabbar {
    display: flex !important; height: 58px;
    background: #202c33; border-top: 1px solid #222d34;
  ...
  .m-tab span:first-child { font-size: 20px; line-height: 1; }
  .m-tab.active { color: #00a884; }

  /* AI悬浮按钮：在tabbar上方，避免被遮挡 */
  .mobile-ai-fab {
