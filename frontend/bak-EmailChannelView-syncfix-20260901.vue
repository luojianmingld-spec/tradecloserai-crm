<template>
  <div class="email-channel" :class="{ 'show-list': mobileView === 'list', 'show-detail': mobileView === 'detail' }">
    ...
    <div class="email-content">
      <!-- 账号栏 -->
      <div class="account-panel" :class="{ collapsed: !isMobile && acctCollapsed }">
        <div class="epanel-header">
          <div class="epanel-brand">
            ...
            <span class="epanel-title" v-if="!acctCollapsed">邮箱</span>
          </div>
          <button v-if="!isMobile" class="col-toggle-btn" @click="acctCollapsed = !acctCollapsed" :title="acctCollapsed ? '展开' : '收起'">
            <svg v-if="acctCollapsed" viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>
        ...
        <template v-if="!acctCollapsed || isMobile">
        <div v-if="accounts.length === 0" class="empty-state">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" stroke-width="1.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
        ...
        <div v-else class="account-list-scroll">
          ...
          >
            <div class="account-avatar" :style="{background: avatarColorFor(acc.email)}">
              {{ (acc.email||'?')[0].toUpperCase() }}
              <span v-if="acc.status === 'connected'" class="acc-online-dot-email"></span>
            </div>
            <div class="account-info">
              <div class="account-email">{{ acc.email }}</div>
              <div class="account-meta">
                <span class="acc-online-text">{{ acc.status === 'connected' ? '在线' : statusLabel(acc.status) }}</span>
        ...
        <!-- 折叠态：圆形✉️按钮 -->
        <div v-if="!isMobile && acctCollapsed" class="acct-collapsed-btn" @click="acctCollapsed = false" title="点击展开">
          <div class="acct-collapsed-circle" style="background:#EA4335;">
      ...
      <div class="email-detail-panel">
        <div v-if="!selectedEmail" class="empty-state">
          ...
          <p style="font-size:13px;color:var(--text-secondary);">选择一封邮件查看详情</p>
        </div>
        <!-- PC端详情（桌面布局） -->
...
const acctCollapsed = ref(localStorage.getItem('email-acct-collapsed') === 'true');
...
function avatarColorFor(key) {
  ...
  const palette = ['var(--accent)','#4FC3F7','#AB47BC','#FF7043','#66BB6A','#FFA726','#EC407A','#26A69A','#EF5350','#5C6BC0'];
  let h = 0;
...
watch(acctCollapsed, v => localStorage.setItem('email-acct-collapsed', v));
onMounted(() => {
  // 60秒自动轮询未读
...
.epanel-title { font-size: 14px; font-weight: 600; color: var(--text-primary); flex: 1; }
.account-panel {
  width: 240px;
  min-width: 240px;
  background: var(--panel-bg);
...
.account-panel.collapsed > *:not(.epanel-header):not(.acct-collapsed-btn) {
  display: none !important;
}
.account-panel.collapsed .epanel-title { display: none; }
.account-panel.collapsed .epanel-brand { flex: 1; justify-content: center; }
.epanel-brand {
...
.col-toggle-btn:hover { background: var(--sidebar-active); color: var(--text-primary); }
.acct-collapsed-btn {
  flex: 1; display: flex; align-items: flex-start; justify-content: center;
  padding-top: 12px; cursor: pointer;
}
.acct-collapsed-circle {
  width: 40px; height: 40px; border-radius: 50%;
  display: flex; align-items: center; justify-content: center;
  transition: transform 0.15s;
}
.acct-collapsed-circle:hover { transform: scale(1.08); }
.account-avatar { position: relative; }
.acc-online-dot-email {
  position: absolute; bottom: 0; right: 0;
  width: 10px; height: 10px; border-radius: 50%;
  background: #00a884; border: 2px solid var(--panel-bg);
}
.acc-online-text { font-size: 11px; color: #00a884; }
...
.account-item:hover { background: var(--panel-header-bg); }
.account-item.active { background: var(--sidebar-active); }
.account-avatar {
...
.account-status.connected { background:var(--accent); color: var(--accent-text); }
...
.account-unread {
  background:var(--accent); color: var(--text-primary); font-size:10px;
...
.email-search::placeholder { color: var(--text-secondary); }
...
.email-item:hover { background: var(--panel-header-bg); }
...
.email-subject { font-size:12px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; margin-top:2px; color:var(--text-primary); }
.email-body-preview { font-size:11px; color:var(--text-secondary); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; margin-top:2px; }
.email-time { font-size:10.5px; color:var(--text-secondary); flex-shrink:0; margin-left:6px; align-self:flex-start; }
...
.meta-label { width:50px; color:var(--text-secondary); flex-shrink:0; }
...
.translation-label { font-size:11.5px; color:var(--accent); font-weight:500; margin-bottom:4px; }
.translation-text { font-size:13px; line-height:1.6; color:var(--text-primary); }
.email-body-scroll { flex:1; overflow-y:auto; padding: 16px; background:var(--panel-bg); }
.email-body {
...
.email-body :deep(a) { color: var(--accent-info); }
.email-reply-area {
...
.ai-replies-header { font-size:12px; font-weight:500; color:var(--accent); margin-bottom:6px; }
.ai-reply-option {
  padding:8px 12px; background:var(--sidebar-active);
  border-radius:8px; margin-bottom:6px; cursor:pointer;
  font-size:12.5px; line-height:1.5; transition: background 0.15s;
  border: 1px solid transparent;
}
.ai-reply-option:hover { background:var(--panel-header-bg); border-color:var(--accent); }
.reply-textarea {
  width:100%; min-height:100px; padding:10px;
  border-radius:8px; border:1px solid #3b4a54;
  background:var(--sidebar-active); color:var(--text-primary);
...
.empty-state {
  ...
  height:100%; gap:10px; color:var(--text-secondary); padding:20px; text-align:center;
}
.empty-state p { margin:0; }
.btn-primary {
  display:inline-flex; align-items:center; gap:6px;
  padding:7px 14px; border:none; border-radius:8px;
  background:var(--accent); color: var(--accent-text); font-size:12.5px;
...
.btn-icon:hover { background:var(--sidebar-active); color:var(--text-primary); }
...
.dialog-box {
  background:var(--panel-header-bg); border-radius:12px;
...
.form-group input,.form-group select {
  ...
  background:var(--sidebar-active); color:var(--text-primary); font-size:13px; outline:none;
}
...
.mobile-back-btn { color:var(--text-primary) !important; }
...
@media (max-width: 768px) {
  ...
  .m-sender-time { font-size: 12px; color: var(--text-secondary); margin-top: 2px; }
  .m-detail-toggle {
  ...
  .m-meta-k { color: var(--text-secondary); flex-shrink: 0; }
  .m-meta-v { color: var(--text-primary); word-break: break-all; flex: 1; }
  ...
  .m-body :deep(a) { color: var(--accent-info); word-break: break-all; }
  .m-body :deep(blockquote) {
    margin: 8px 0; padding: 8px 12px;
    border-left: 3px solid #3b4a54;
    background: var(--panel-header-bg); color: var(--text-secondary);
  ...
  .m-fab-translate:active { background: var(--sidebar-active); }
  ...
  .m-trans-label { font-size: 13px; color: var(--accent); font-weight: 600; }
  .m-trans-toggle {
    background: transparent;
  ...
  .m-trans-toggle:active { background: var(--sidebar-active); }
  .m-trans-body-inline {
    font-size: 14.5px;
    line-height: 1.7;
    color: var(--text-primary);
  ...
  .m-bar-btn.m-bar-primary { color: var(--accent); }
  ...
  .m-more-item:active { background: var(--panel-header-bg); }
  ...
  .m-field-k { color: var(--text-secondary); flex-shrink: 0; margin-right: 6px; }
  .m-field-v { color: var(--text-primary); flex: 1; word-break: break-all; }
  ...
  .m-quote-head { font-size: 12px; color: var(--text-secondary); margin-bottom: 8px; font-weight: 600; }
  .m-quote-line { font-size: 12px; color: var(--text-secondary); padding: 2px 0; }
  .m-quote-body {
