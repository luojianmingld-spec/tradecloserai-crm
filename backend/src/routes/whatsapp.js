...
router.get('/messages', async (req, res) => {
  try {
    ...
    const out = messages.reverse().map(m => ({ ...m, platform }));
    res.json(out);
  } catch (err) {
...
async function ensureConversation(accountId, jid, platform = "whatsapp") {
  try {
    ...
    if (!contact) {
      ...
      try {
        contact = await prisma.contact.create({
          data: { accountId, platform, jid, phone: platform === "whatsapp" ? phone : null },
        });
      } catch (e) {
...
router.get('/conversations', async (req, res) => {
  try {
    ...
    let sessionIds;
    if (reqPlatform === 'whatsapp') sessionIds = waSessionIds;
    else if (reqPlatform === 'telegram') sessionIds = tgSessionIds;
    ...
    // WA客户phone数字精确匹配，TG也按chatId(纯数字)匹配phone字段
    const customers = phones.length
      ? await prisma.customer.findMany({
          where: { userId: req.userId, OR: [
            { phone: { in: phones } },
            { jid: { in: [...contactMap.keys()] } },
          ]},
    ...
    for (const [jid, entry] of contactMap) {
      const phone = jid.split('@')[0];
      const platform = entry.platform || 'whatsapp';
      // 过滤无效jid：WA必须是5+位纯数字；TG是纯数字chatId（可以长点）
