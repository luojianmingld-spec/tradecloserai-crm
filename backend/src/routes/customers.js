/**
 * customers.js - Append new customer-management routes to existing file.
 * Strategy: rewrite customers.js keeping all existing by-jid / legacy logic,
 * and override the generic GET '/', GET '/:id', POST '/', PUT '/:id', DELETE '/:id'
 * with richer versions that include KPIs/documents/followUps. We keep the by-jid
 * routes BEFORE these wildcard routes (Express matches in declaration order),
 * and make '/:id' numeric-only to avoid clashing with by-jid.
 */

import { Router } from 'express';
import { autoBantScore } from './bant-score.js';
import { PrismaClient } from '@prisma/client';
import { chatComplete, resolveModelId } from '../services/ai.service.js';
import { chatCompleteLite } from '../services/ai-client.js';
import { numberToWords } from '../utils/numberToWords.js';
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import https from "https";
import http from "http";
import { getEvolutionConnector } from "../services/evolution-connector.js";
import { runFullBackgroundCheck } from "./background-check.js";
import { resolveToPhoneJid } from "../services/lid-mapping.js";
import { inferCountry } from "../utils/phone-country.js";
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const DATA_DIR = path.resolve(__dirname, '../../data');
const SELLER_INFO_PATH = path.join(DATA_DIR, 'seller-info.json');
function loadSellerInfo() {
  try { return JSON.parse(fs.readFileSync(SELLER_INFO_PATH, 'utf-8')); } catch { return {}; }
}

const router = Router();
const prisma = new PrismaClient();

// ─── Allowed fields & helpers ───
const JID_WRITABLE_FIELDS = [
  'name', 'companyName', 'contactName', 'title', 'email', 'website',
  'country', 'city', 'phone', 'notes', 'tags', 'source', 'customerLevel',
  'firstContactAt', 'lastContactAt', 'address', 'industry', 'status',
  // 需求总结
  'requirementSummary', 'requirementProducts', 'requirementBudget',
  'requirementQuantity', 'requirementDelivery', 'requirementSource',
  'dealStage', 'dealStageAt', 'dealValue',
  'isBusiness',
];

// Fields allowed for :id CRUD (wider set)
const ID_WRITABLE_FIELDS = [
  'name', 'companyName', 'contactName', 'title', 'email', 'website',
  'country', 'city', 'phone', 'notes', 'tags', 'source', 'customerLevel',
  'firstContactAt', 'lastContactAt', 'address', 'industry', 'status',
  'jid', 'company',
  // 需求总结
  'requirementSummary', 'requirementProducts', 'requirementBudget',
  'requirementQuantity', 'requirementDelivery',
  'dealStage', 'dealStageAt', 'dealValue',
  'isBusiness',
];

// 需求字段列表（PUT 时自动打时间戳）
const REQUIREMENT_FIELDS = [
  'requirementSummary', 'requirementProducts', 'requirementBudget',
  'requirementQuantity', 'requirementDelivery',
];

function safeParseJSON(str, fallback) {
  if (str == null) return fallback;
  try { return JSON.parse(str); } catch { return fallback; }
}

function serializeCustomer(c) {
  if (!c) return null;
  return {
    ...c,
    tags: typeof c.tags === 'string' ? safeParseJSON(c.tags, []) : (c.tags || []),
  };
}

function decodeJid(param) {
  if (!param) return null;
  let jid = decodeURIComponent(String(param)).trim();
  // 归一化：裸号补 @s.whatsapp.net 域（Baileys/WA 标准）
  if (jid && !jid.includes('@')) {
    jid = jid + '@s.whatsapp.net';
  }
  return jid;
}

/** 归一化 JID 用于去重匹配：返回标准格式 + 备用格式数组 */
function normalizeJid(jid) {
  if (!jid) return [];
  const clean = String(jid).trim();
  const variants = new Set([clean]);
  if (clean.includes('@s.whatsapp.net')) {
    variants.add(clean.replace('@s.whatsapp.net', '@c.us'));
    variants.add(clean.replace('@s.whatsapp.net', '@lid'));
  } else if (clean.includes('@c.us')) {
    variants.add(clean.replace('@c.us', '@s.whatsapp.net'));
    variants.add(clean.replace('@c.us', '@lid'));
  } else if (clean.includes('@lid')) {
    variants.add(clean.replace('@lid', '@s.whatsapp.net'));
    variants.add(clean.replace('@lid', '@c.us'));
  } else if (!clean.includes('@')) {
    variants.add(clean + '@s.whatsapp.net');
    variants.add(clean + '@c.us');
    variants.add(clean + '@lid');
  }
  return [...variants];
}

/** 按 userId + jid 查找客户（兼容多种jid格式，防止重复客户） */
async function findCustomerByJid(prisma, userId, jid) {
  const variants = normalizeJid(jid);
  if (!variants.length) return null;
  return prisma.customer.findFirst({
    where: { userId, jid: { in: variants } },
  });
}

// ============ 按 JID 的接口（客户背调 V1 - 保持原样） ============

router.get('/by-jid/:jid', async (req, res) => {
  try {
    const jid = decodeJid(req.params.jid);
    if (!jid) return res.status(400).json({ error: 'jid required' });
    const userId = req.userId;

    // L2: LID 解析 —— @lid 先解析为真实手机号JID，避免同一人（@lid与真实号）重复建档
    const realJid = jid.includes('@lid') ? resolveToPhoneJid(jid) : jid;
    const lookupJids = [...new Set([jid, realJid])];

    let customer = null;
    for (const j of lookupJids) {
      customer = await findCustomerByJid(prisma, userId, j);
      if (customer) break;
    }

    if (!customer) {
      // L2: 空会话不自动建档 —— 两个jid下均无任何消息时返回404（杜绝"看一眼就建档"的幽灵客户）
      const msgWhere = { OR: [{ from: { in: lookupJids } }, { to: { in: lookupJids } }] };
      const msgCount = await prisma.wAMessage.count({ where: msgWhere });
      if (msgCount === 0) {
        return res.status(404).json({ error: 'Customer not found', code: 'NO_MESSAGES' });
      }
      const firstMsg = await prisma.wAMessage.findFirst({ where: msgWhere, orderBy: { timestamp: 'asc' } });
      const lastMsg = await prisma.wAMessage.findFirst({ where: msgWhere, orderBy: { timestamp: 'desc' } });
      const phone = realJid.split('@')[0] || null;
      let defaultName = phone || '';
      try {
        // 姓名从Contact表取：realJid优先，其次原jid
        let contact = await prisma.contact?.findFirst?.({ where: { jid: realJid } });
        if (!contact && realJid !== jid) contact = await prisma.contact?.findFirst?.({ where: { jid } });
        if (contact?.name || contact?.pushName) defaultName = contact.name || contact.pushName || defaultName;
      } catch {}

      customer = await prisma.customer.create({
        data: {
          userId, jid: realJid, name: defaultName, phone, customerLevel: 'C',
          country: inferCountry(phone),
          source: 'whatsapp', status: 'potential', intentLevel: 5, assignedTo: userId,
          firstContactAt: firstMsg?.timestamp || new Date(),
          lastContactAt: lastMsg?.timestamp || new Date(),
        },
      });
    } else {
      const patch = {};
      if (!customer.firstContactAt || !customer.lastContactAt) {
        const msgWhere = { OR: [{ from: { in: lookupJids } }, { to: { in: lookupJids } }] };
        const firstMsg = await prisma.wAMessage.findFirst({ where: msgWhere, orderBy: { timestamp: 'asc' } });
        const lastMsg = await prisma.wAMessage.findFirst({ where: msgWhere, orderBy: { timestamp: 'desc' } });
        if (!customer.firstContactAt && firstMsg) patch.firstContactAt = firstMsg.timestamp;
        if (!customer.lastContactAt && lastMsg) patch.lastContactAt = lastMsg.timestamp;
        if (Object.keys(patch).length) {
          customer = await prisma.customer.update({ where: { id: customer.id }, data: patch });
        }
      }
    }

    res.json(serializeCustomer(customer));
  } catch (err) {
    console.error('[Customers] by-jid GET error:', err);
    res.status(500).json({ error: 'Failed to get customer' });
  }
});

router.put('/by-jid/:jid', async (req, res) => {
  try {
    const jid = decodeJid(req.params.jid);
    if (!jid) return res.status(400).json({ error: 'jid required' });
    const userId = req.userId;

    let customer = await findCustomerByJid(prisma, userId, jid);
    if (!customer) {
      const phone = jid.split('@')[0] || null;
      customer = await prisma.customer.create({
        data: {
          userId, jid, name: phone || '', phone, customerLevel: 'C', source: 'whatsapp',
          status: 'potential', intentLevel: 5, assignedTo: userId,
          firstContactAt: new Date(), lastContactAt: new Date(),
        },
      });
    }

    const data = {};
    for (const field of JID_WRITABLE_FIELDS) {
      if (req.body[field] !== undefined) {
        if (field === 'tags') {
          const v = req.body[field];
          data[field] = Array.isArray(v) ? JSON.stringify(v) : String(v);
        } else {
          data[field] = req.body[field];
        }
      }
    }
    // Auto-set requirement timestamps
    const reqFields = ['requirementSummary','requirementProducts','requirementBudget','requirementQuantity','requirementDelivery','requirementSource'];
    if (reqFields.some(f => data[f] !== undefined)) {
      data.requirementUpdatedAt = new Date();
      if (!data.requirementSource) data.requirementSource = 'manual';
    }
    if (data.companyName !== undefined) data.company = data.companyName;

    const updated = await prisma.customer.update({ where: { id: customer.id }, data });
    res.json(serializeCustomer(updated));
  } catch (err) {
    console.error('[Customers] by-jid PUT error:', err);
    res.status(500).json({ error: 'Failed to update customer' });
  }
});

router.post('/by-jid/:jid/extract-ai', async (req, res) => {
  try {
    const jid = decodeJid(req.params.jid);
    if (!jid) return res.status(400).json({ error: 'jid required' });
    const userId = req.userId;
    const limit = Math.min(parseInt(req.body?.limit) || 20, 50);

    const messages = await prisma.wAMessage.findMany({
      where: { OR: [{ from: jid }, { to: jid }] },
      orderBy: { timestamp: 'desc' },
      take: limit,
    });
    const context = messages.slice().reverse().map(m => {
      const who = (m.direction === 'outbound' || m.direction === 'outgoing') ? '我(业务员)' : '客户';
      const ts = m.timestamp ? new Date(m.timestamp).toISOString().slice(0, 16).replace('T', ' ') : '';
      return `[${ts}] ${who}: ${m.body || ''}`;
    }).join('\n');

    if (!context.trim()) {
      return res.json({
        companyName: null, contactName: null, country: null, title: null,
        email: null, website: null, source: '未知', notes: '暂无聊天记录', address: null, industry: null,
      });
    }

    const modelId = await resolveModelId();
    const systemPrompt = `你是一个外贸客户信息抽取助手。根据以下WhatsApp聊天记录，抽取客户关键信息。

## 抽取字段
- companyName: 客户公司名（英文/原文），没有则null
- contactName: 客户联系人名/称呼，没有则null
- country: 客户国家/地区，没有则null
- title: 客户职位/头衔，没有则null
- email: 邮箱地址，没有则null
- website: 网址，没有则null
- address: 客户地址，没有则null
- industry: 行业，没有则null
- source: 客户来源推测（展会/独立站询盘/主动开发/老客户推荐/其他/未知）
- notes: 其他关键信息摘要（一句话，中文）

要求：
- 严格只输出JSON，不要markdown代码块，不要其他文字
- 不确定的字段设null，不要瞎猜
- 格式：{"companyName":"...","contactName":"...","country":"...","title":"...","email":"...","website":"...","address":"...","industry":"...","source":"...","notes":"..."}`;

    const userPrompt = `聊天记录（最近${messages.length}条）：\n${context}\n\n请抽取：`;
    const raw = await chatComplete(modelId, systemPrompt, userPrompt, { temperature: 0.2, maxTokens: 900 });

    let parsed = null;
    try {
      const m = raw.match(/\{[\s\S]*\}/);
      if (m) parsed = JSON.parse(m[0]);
    } catch (e) {
      console.error('[Customers] extract-ai JSON parse error:', e.message, raw);
    }
    if (!parsed || typeof parsed !== 'object') {
      parsed = { companyName: null, contactName: null, country: null, title: null, email: null, website: null, address: null, industry: null, source: '未知', notes: 'AI解析失败' };
    }
    const safe = {
      companyName: parsed.companyName || null,
      contactName: parsed.contactName || null,
      country: parsed.country || null,
      title: parsed.title || null,
      email: parsed.email || null,
      website: parsed.website || null,
      address: parsed.address || null,
      industry: parsed.industry || null,
      source: parsed.source || '未知',
      notes: parsed.notes || null,
    };
    await prisma.customer.updateMany({ where: { userId, jid }, data: { aiExtractedAt: new Date() } });
    res.json(safe);
  } catch (err) {
    console.error('[Customers] extract-ai error:', err);
    res.status(500).json({ error: 'AI抽取失败' });
  }
});

router.post('/by-jid/:jid/ask-company', async (req, res) => {
  try {
    const jid = decodeJid(req.params.jid);
    const userId = req.userId;
    const limit = Math.min(parseInt(req.body?.limit) || 20, 50);
    const messages = await prisma.wAMessage.findMany({
      where: { OR: [{ from: jid }, { to: jid }] },
      orderBy: { timestamp: 'desc' },
      take: limit,
    });
    const context = messages.slice().reverse().map(m => {
      const who = (m.direction === 'outbound' || m.direction === 'outgoing') ? '我' : '客户';
      return `${who}: ${m.body || ''}`;
    }).join('\n');

    const modelId = await resolveModelId();
    const systemPrompt = `你是外贸业务员助手。我还不知道客户的公司名，请根据聊天记录生成2句自然的询问话术，礼貌地了解对方公司信息。

要求：
- 用客户当前使用的语言（从聊天记录判断，英语/阿语/西语/法语等）
- 自然不生硬，不要像问卷
- 给2个版本："自然随意版"、"礼貌正式版"
- 严格只输出JSON，不要markdown：{"replies":[{"name":"自然随意版","foreign":"外文原文","chinese":"中文翻译"},{"name":"礼貌正式版","foreign":"外文原文","chinese":"中文翻译"}]}`;
    const userPrompt = `聊天记录：\n${context || '(暂无)'}\n\n请生成话术。`;
    const raw = await chatComplete(modelId, systemPrompt, userPrompt, { temperature: 0.8, maxTokens: 600 });

    let parsed = null;
    try { const m = raw.match(/\{[\s\S]*\}/); if (m) parsed = JSON.parse(m[0]); } catch {}
    if (!parsed || !Array.isArray(parsed.replies)) {
      parsed = { replies: [
        { name: '自然随意版', foreign: 'By the way, may I know which company you are from?', chinese: '顺便问一下，您是哪家公司的？' },
        { name: '礼貌正式版', foreign: 'Could you please let me know your company name so I can better assist you?', chinese: '请问您能告诉我贵公司名称吗，以便我更好地为您服务？' },
      ]};
    }
    res.json(parsed);
  } catch (err) {
    console.error('[Customers] ask-company error:', err);
    res.status(500).json({ error: '生成失败' });
  }
});

router.post('/by-jid/:jid/sync-business-profile', async (req, res) => {
  try {
    res.json({ success: false, message: 'Evolution API Business Profile同步开发中' });
  } catch (err) {
    console.error('[Customers] sync-business-profile error:', err);
    res.status(500).json({ error: '同步失败' });
  }
});

// ─── Find customer id by jid (used by chat → customer jump) ───
router.get('/id-by-jid/:jid', async (req, res) => {
  try {
    const jid = decodeJid(req.params.jid);
    if (!jid) return res.status(400).json({ error: 'jid required' });
    const cust = await prisma.customer.findFirst({ where: { userId: req.userId, jid } });
    if (!cust) return res.status(404).json({ error: 'not found' });
    res.json({ id: cust.id });
  } catch (err) {
    console.error('[Customers] id-by-jid error:', err);
    res.status(500).json({ error: 'query failed' });
  }
});

// ============ 新客户管理模块 API（数字 ID 路由） ============

// Helper: compute KPIs（【TC-004修复】应用与列表一致的 where 筛选，保证 stats 与 items 口径统一）
async function computeKPIs(userId, baseWhere = {}) {
  const now = new Date();
  const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
  const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);

  const [total, active, aLevel, bLevel, dLevel, pending, newThisMonth] = await Promise.all([
    prisma.customer.count({ where: baseWhere }),
    prisma.customer.count({ where: { ...baseWhere, status: { in: ['active', 'following'] }, lastContactAt: { gte: thirtyDaysAgo } } }),
    prisma.customer.count({ where: { ...baseWhere, customerLevel: 'A' } }),
    prisma.customer.count({ where: { ...baseWhere, customerLevel: 'B' } }),
    prisma.customer.count({ where: { ...baseWhere, customerLevel: 'D' } }),
    prisma.customer.count({ where: { ...baseWhere, status: 'potential' } }),
    prisma.customer.count({ where: { ...baseWhere, createdAt: { gte: firstDayOfMonth } } }),
  ]);
  const cLevel = Math.max(0, total - aLevel - bLevel - dLevel);
  return { total, active, aLevel, bLevel, cLevel, dLevel, pending, newThisMonth };
}

// List customers with filter/search/pagination + KPI
router.get('/', async (req, res) => {
  try {
    const userId = req.userId;
    const { level, status: statusQ, source, country, search, page, pageSize, sort, order } = req.query;
    // 排除自己的WA号（不把自己当客户）
    const where = { userId };
    // Self-exclusion using notIn (avoids Prisma NOT+NULL bug)
    // 异步获取selfJid，带超时，不阻塞主查询
    let selfJid = null;
    try {
      selfJid = await Promise.race([
        (async () => getEvolutionConnector().getOwnerJid?.())(),
        new Promise(r => setTimeout(() => r(null), 3000)),
      ]);
    } catch (e) { /* 不阻塞 */ }
    if (selfJid) {
      const otherDomain = selfJid.includes("@s.whatsapp.net")
        ? selfJid.replace("@s.whatsapp.net", "@c.us")
        : selfJid.replace("@c.us", "@s.whatsapp.net");
      // 兼容 jid NULL：NULL NOT IN (...) 在 SQL 中为 UNKNOWN，会把 email/manual 客户（jid=NULL）误过滤
      where.AND = [{ OR: [{ jid: null }, { jid: { notIn: [selfJid, otherDomain] } }] }];
    }
    where.source = { not: "self" };
    if (level && ['A', 'B', 'C', 'D'].includes(level.toUpperCase())) where.customerLevel = level.toUpperCase();
    if (statusQ) where.status = statusQ;
    if (source) where.source = source;
    if (country) where.country = { contains: country, mode: 'insensitive' };
    // isBusiness filter: true/false/null（客户管理列表只显示确认的业务客户：前端固定传 true）
    if (req.query.isBusiness !== undefined && req.query.isBusiness !== '') {
      const ib = req.query.isBusiness;
      if (ib === 'null') where.isBusiness = null;
      else if (ib === 'true') where.isBusiness = true;
      else if (ib === 'false') where.isBusiness = false;
    }
    if (search) {
      where.OR = [
        { name: { contains: search } }, { phone: { contains: search } },
        { email: { contains: search } }, { company: { contains: search } },
        { companyName: { contains: search } }, { country: { contains: search } },
        { contactName: { contains: search } }, { jid: { contains: search } },
      ];
    }

    // "最近联系"快捷筛选
    if (req.query.recent === '1') {
      where.lastContactAt = { not: null };
    }

    const _page = Math.max(parseInt(page) || 1, 1);
    const _size = Math.min(parseInt(pageSize) || 50, 200);
    const orderBy = sort ? { [sort]: order === 'asc' ? 'asc' : 'desc' } : { updatedAt: 'desc' };

    const [customers, total, kpis] = await Promise.all([
      prisma.customer.findMany({
        where, orderBy, skip: (_page - 1) * _size, take: _size,
        include: {
          _count: { select: { documents: true, followUps: true } },
        },
      }),
      prisma.customer.count({ where }),
      computeKPIs(userId, where),
    ]);

    // Enrich with last message preview / pending doc / follow-up counts
    const jidSet = [...new Set(customers.map(c=>c.jid).filter(Boolean))];
    const lastMsgMap = new Map();
    // Batch query: fetch last messages for ALL jids in one query using groupBy
    if (jidSet.length > 0 && jidSet.length <= 200) {
      try {
        // Get recent messages for all jids in one query
        const recentMsgs = await prisma.wAMessage.findMany({
          where: {
            OR: [
              { from: { in: jidSet } },
              { to: { in: jidSet } },
            ],
          },
          orderBy: { timestamp: 'desc' },
          select: { from: true, to: true, body: true, timestamp: true, direction: true },
          take: jidSet.length * 3, // get a few per jid, pick latest below
        });
        // For each jid, find the latest message
        for (const jid of jidSet) {
          const match = recentMsgs.find(m => m.from === jid || m.to === jid);
          if (match) lastMsgMap.set(jid, match);
        }
      } catch(e) {
        console.error('[Customers] lastMsg batch error:', e.message);
      }
    }

    // Pending doc counts (per customer)
    const idList = customers.map(c=>c.id);
    const pendingMap = new Map();
    try {
      const docs = await prisma.document.findMany({
        where: { customerId: { in: idList } },
        select: { customerId: true, status: true },
      });
      for (const d of docs) {
        const ent = pendingMap.get(d.customerId) || { count:0, pending:0 };
        ent.count++;
        if (d.status === 'DRAFT') ent.pending++;
        pendingMap.set(d.customerId, ent);
      }
    } catch(e) { console.error('docAgg err', e.message); }

    const list = customers.map(c => serializeCustomer(c)).map(c => {
      const docEnt = pendingMap.get(c.id) || { count: c._count?.documents||0, pending:0 };
      const lastMsg = c.jid ? lastMsgMap.get(c.jid) : null;
      let lastMessage = null;
      if (lastMsg && lastMsg.body) {
        const b = String(lastMsg.body).replace(/\s+/g,' ').trim();
        lastMessage = b.length > 40 ? b.slice(0,40)+'…' : b;
      }
      return {
        ...c,
        _docCount: Number(docEnt.count || 0),
        _pendingDocCount: Number(docEnt.pending || 0),
        _followUpCount: Number(c._count?.followUps || 0),
        _lastMessage: lastMessage,
        _lastMessageAt: lastMsg?.timestamp || c.lastContactAt || null,
        _lastMessageDir: lastMsg?.direction || null,
      };
    });

    res.json({
      items: list,
      total,
      page: _page,
      pageSize: _size,
      stats: kpis,
    });
  } catch (err) {
    console.error('[Customers] List error:', err);
    res.status(500).json({ error: 'Failed to list customers' });
  }
});

// ============ L2: Pending Business Confirmation ============
// GET /pending-business-check - Get contacts with isBusiness=null that have messages
router.get('/pending-business-check', async (req, res) => {
  try {
    const userId = req.userId;
    const customers = await prisma.customer.findMany({
      where: {
        userId,
        isBusiness: null,
        jid: { not: { endsWith: '@g.us' } },
      },
      select: {
        id: true,
        jid: true,
        name: true,
        phone: true,
        companyName: true,
        email: true,
      },
      orderBy: { lastContactAt: 'desc' },
      take: 20,
    });

    // Filter to only those who have at least one message
    // LID增强：消息可能落在 @lid 或真实号任一 jid 下，两个都要统计，避免漏统计
    const result = [];
    for (const c of customers) {
      const realJid = c.jid && c.jid.includes('@lid') ? resolveToPhoneJid(c.jid) : c.jid;
      const jids = [...new Set([c.jid, realJid].filter(Boolean))];
      const msgCount = await prisma.wAMessage.count({
        where: { OR: [{ from: { in: jids } }, { to: { in: jids } }] },
      });
      if (msgCount > 0) {
        result.push({
          id: c.id,
          jid: c.jid,
          name: c.name,
          phone: c.phone,
          companyName: c.companyName,
          email: c.email,
          displayName: c.name || c.phone || c.jid?.split('@')[0] || 'Unknown',
        });
      }
    }

    res.json({ pending: result, count: result.length });
  } catch (err) {
    console.error('[pending-business-check] error:', err);
    res.status(500).json({ error: 'Failed to fetch pending contacts' });
  }
});

// PUT /pending-business-check/:id - Update business confirmation status
router.put('/pending-business-check/:id', async (req, res) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return res.status(400).json({ error: 'Invalid id' });
    const id = parseInt(req.params.id);
    const { isBusiness } = req.body;
    if (typeof isBusiness !== 'boolean') return res.status(400).json({ error: 'isBusiness must be boolean' });

    const updated = await prisma.customer.update({
      where: { id },
      data: {
        isBusiness,
        businessDetectedAt: new Date(),
      },
    });
    res.json({ ok: true, customer: serializeCustomer(updated) });
  } catch (err) {
    console.error('[pending-business-check] update error:', err);
    res.status(500).json({ error: 'Failed to update business status' });
  }
});

// Get single customer by numeric id (with stats) - must come AFTER /by-jid & /by-phone etc.
router.get('/:id', async (req, res, next) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return next('route');
    const id = parseInt(req.params.id);
    const customer = await prisma.customer.findUnique({ where: { id } });
    if (!customer) return res.status(404).json({ error: 'Customer not found' });

    const [docCount, followUpCount, messageCount, lastMessages] = await Promise.all([
      prisma.document.count({ where: { customerId: id } }),
      prisma.customerFollowUp.count({ where: { customerId: id } }),
      customer.jid ? prisma.wAMessage.count({ where: { OR: [{ from: customer.jid }, { to: customer.jid }] } }) : 0,
      customer.jid ? prisma.wAMessage.findMany({
        where: { OR: [{ from: customer.jid }, { to: customer.jid }] },
        orderBy: { timestamp: 'desc' }, take: 20,
      }) : [],
    ]);

    // Latest pending doc hint
    const pendingDoc = await prisma.document.findFirst({
      where: { customerId: id, status: 'DRAFT' },
      orderBy: { updatedAt: 'desc' },
      select: { id: true, docNumber: true, type: true, updatedAt: true, totalAmount: true },
    });

    res.json({
      ...serializeCustomer(customer),
      _stats: { documentCount: docCount, followUpCount, messageCount },
      _pendingDoc: pendingDoc,
      _lastMessages: lastMessages.map(m => ({
        id: m.id, direction: m.direction, body: m.body, timestamp: m.timestamp, type: m.type,
      })),
    });
  } catch (err) {
    console.error('[Customers] Get error:', err);
    res.status(500).json({ error: 'Failed to get customer' });
  }
});

// Create customer (manual)
router.post('/', async (req, res) => {
  try {
    const userId = req.userId;
    const b = req.body || {};
    const displayName = b.companyName || b.name || b.contactName;
    if (!displayName) return res.status(400).json({ error: '请填写公司名或联系人姓名' });

    const data = {
      userId,
      name: b.name || displayName,
      jid: b.jid || null,
      phone: b.phone || null,
      email: b.email || null,
      company: b.company || b.companyName || null,
      companyName: b.companyName || b.company || null,
      contactName: b.contactName || null,
      title: b.title || null,
      website: b.website || null,
      country: b.country || null,
      address: b.address || null,
      industry: b.industry || null,
      tags: Array.isArray(b.tags) ? JSON.stringify(b.tags) : (b.tags || '[]'),
      notes: b.notes || null,
      source: b.source || 'manual',
      customerLevel: b.customerLevel || 'C',
      status: b.status || 'new',
      intentLevel: b.intentLevel || 5,
      assignedTo: userId,
      isBusiness: true, // L2: 用户手动建档 = 明确动作，直接视为业务客户
      firstContactAt: b.firstContactAt ? new Date(b.firstContactAt) : new Date(),
      lastContactAt: b.lastContactAt ? new Date(b.lastContactAt) : new Date(),
    };

    const customer = await prisma.customer.create({ data });
    res.status(201).json(serializeCustomer(customer));
  } catch (err) {
    console.error('[Customers] Create error:', err);
    res.status(500).json({ error: 'Failed to create customer: ' + err.message });
  }
});

// Update customer by numeric id
router.put('/:id', async (req, res, next) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return next('route');
    const id = parseInt(req.params.id);
    const existing = await prisma.customer.findUnique({ where: { id } });
    if (!existing) return res.status(404).json({ error: 'Customer not found' });

    const data = {};
    for (const field of ID_WRITABLE_FIELDS) {
      if (req.body[field] !== undefined) {
        if (field === 'tags') {
          const v = req.body[field];
          data[field] = Array.isArray(v) ? JSON.stringify(v) : String(v);
        } else if (['firstContactAt', 'lastContactAt'].includes(field)) {
          data[field] = req.body[field] ? new Date(req.body[field]) : null;
        } else {
          data[field] = req.body[field];
        }
      }
    }
    if (data.companyName && !data.company) data.company = data.companyName;

    // 如果更新了任何需求字段，自动更新时间戳
    const hasRequirementField = REQUIREMENT_FIELDS.some(f => f in data);
    if (hasRequirementField) {
      data.requirementUpdatedAt = new Date();
      // 未显式指定来源则标记为 manual
      if (!data.requirementSource) data.requirementSource = 'manual';
    }

    const customer = await prisma.customer.update({ where: { id }, data });
    res.json(serializeCustomer(customer));
  } catch (err) {
    console.error('[Customers] Update error:', err);
    res.status(500).json({ error: 'Failed to update customer: ' + err.message });
  }
});

// Delete customer
router.delete('/:id', async (req, res, next) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return next('route');
    const id = parseInt(req.params.id);
    const existing = await prisma.customer.findUnique({ where: { id } });
    if (!existing) return res.status(404).json({ error: 'Customer not found' });
    await prisma.customer.delete({ where: { id } });
    res.json({ success: true });
  } catch (err) {
    console.error('[Customers] Delete error:', err);
    res.status(500).json({ error: 'Failed to delete customer' });
  }
});

// ─── Documents for a customer ───
router.get('/:id/documents', async (req, res, next) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return next('route');
    const id = parseInt(req.params.id);
    const { type } = req.query;
    const where = { customerId: id };
    if (type) where.type = type.toUpperCase();
    const docs = await prisma.document.findMany({
      where,
      include: { items: true },
      orderBy: { createdAt: 'desc' },
    });
    // Stats
    const [total, totalAmt, pending] = await Promise.all([
      prisma.document.count({ where: { customerId: id } }),
      prisma.document.aggregate({ where: { customerId: id, status: { not: 'DRAFT' } }, _sum: { totalAmount: true } }),
      prisma.document.count({ where: { customerId: id, status: 'DRAFT' } }),
    ]);
    res.json({
      items: docs.map(d => ({
        ...d,
        sellerInfo: typeof d.sellerInfo === 'string' ? JSON.parse(d.sellerInfo) : d.sellerInfo,
        buyerInfo: typeof d.buyerInfo === 'string' ? JSON.parse(d.buyerInfo) : d.buyerInfo,
      })),
      stats: { total, totalAmount: totalAmt._sum.totalAmount || 0, pending },
    });
  } catch (err) {
    console.error('[Customers] documents error:', err);
    res.status(500).json({ error: 'Failed to load documents' });
  }
});

// ─── Follow-ups ───
router.get('/:id/follow-ups', async (req, res, next) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return next('route');
    const id = parseInt(req.params.id);
    const items = await prisma.customerFollowUp.findMany({
      where: { customerId: id }, orderBy: { createdAt: 'desc' },
    });
    res.json(items.map(it => ({
      ...it,
      metadata: it.metadata ? safeParseJSON(it.metadata, {}) : {},
    })));
  } catch (err) {
    console.error('[Customers] follow-ups GET error:', err);
    res.status(500).json({ error: 'Failed to load follow-ups' });
  }
});

router.post('/:id/follow-ups', async (req, res, next) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return next('route');
    const id = parseInt(req.params.id);
    const { content, source, metadata } = req.body || {};
    if (!content) return res.status(400).json({ error: 'content required' });
    const fu = await prisma.customerFollowUp.create({
      data: {
        customerId: id,
        content,
        source: source || 'manual',
        metadata: metadata ? JSON.stringify(metadata) : null,
      },
    });
    res.status(201).json({ ...fu, metadata: fu.metadata ? safeParseJSON(fu.metadata, {}) : {} });
  } catch (err) {
    console.error('[Customers] follow-ups POST error:', err);
    res.status(500).json({ error: 'Failed to add follow-up' });
  }
});

router.delete('/follow-ups/:fid', async (req, res) => {
  try {
    await prisma.customerFollowUp.delete({ where: { id: req.params.fid } });
    res.json({ success: true });
  } catch (err) {
    console.error('[Customers] follow-up DELETE error:', err);
    res.status(500).json({ error: 'delete failed' });
  }
});

// ─── AI follow-up generation ───
router.post('/:id/generate-followups', async (req, res, next) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return next('route');
    const id = parseInt(req.params.id);
    const customer = await prisma.customer.findUnique({ where: { id } });
    if (!customer) return res.status(404).json({ error: 'Customer not found' });
    if (!customer.jid) return res.status(400).json({ error: '该客户无WhatsApp JID，无法生成' });

    const limit = Math.min(parseInt(req.body?.limit) || 300, 800);
    const messages = await prisma.wAMessage.findMany({
      where: { OR: [{ from: customer.jid }, { to: customer.jid }] },
      orderBy: { timestamp: 'asc' },
      take: limit,
    });
    if (!messages.length) {
      return res.status(400).json({ error: '暂无聊天记录，无法生成跟进' });
    }

    // Group into daily segments
    const segments = []; // { date: YYYY-MM-DD, msgs: [] }
    for (const m of messages) {
      if (!m.body || !m.timestamp) continue;
      const d = new Date(m.timestamp);
      const key = d.toISOString().slice(0, 10);
      let seg = segments.find(s => s.date === key);
      if (!seg) { seg = { date: key, msgs: [] }; segments.push(seg); }
      seg.msgs.push(m);
    }

    const modelId = await resolveModelId();
    const systemPrompt = `你是一个外贸业务跟进记录总结助手。根据与客户的WhatsApp聊天记录（按天分段），用中文总结当天的关键沟通要点，100字以内，包含：客户需求/询盘内容、报价情况、异议/问题、承诺的下一步动作。只输出中文总结文本，不要任何JSON/格式。如果当天无实质沟通内容，回复"无实质沟通"。`;

    const generated = [];
    for (const seg of segments) {
      const context = seg.msgs.slice(-30).map(m => {
        const who = (m.direction === 'outbound' || m.direction === 'outgoing') ? '我' : '客户';
        const ts = new Date(m.timestamp).toISOString().slice(11, 16);
        return `[${ts}] ${who}: ${(m.body || '').slice(0, 500)}`;
      }).join('\n');
      let summary = '';
      try {
        summary = await chatComplete(modelId, systemPrompt,
          `日期：${seg.date}，共${seg.msgs.length}条消息\n聊天记录：\n${context}\n\n请总结：`,
          { temperature: 0.3, maxTokens: 220 });
        summary = (summary || '').trim();
      } catch (e) {
        console.error('[Customers] generate-followups segment error:', e.message);
        summary = '（AI总结失败）';
      }
      if (summary && !/无实质沟通/.test(summary)) {
        const content = `${seg.date}：${summary.replace(/^["""]|["""]$/g, '')}`;
        generated.push({
          date: seg.date,
          content,
          msgCount: seg.msgs.length,
        });
      }
    }

    // Replace existing ai_summary entries for this customer
    await prisma.customerFollowUp.deleteMany({ where: { customerId: id, source: 'ai_summary' } });
    const created = [];
    for (const g of generated) {
      const fu = await prisma.customerFollowUp.create({
        data: {
          customerId: id,
          content: g.content,
          source: 'ai_summary',
          metadata: JSON.stringify({ date: g.date, msgCount: g.msgCount, generatedAt: new Date().toISOString() }),
          createdAt: new Date(g.date + 'T12:00:00Z'),
        },
      });
      created.push(fu);
    }

    res.json({ success: true, generated: created.length, items: created });
  } catch (err) {
    console.error('[Customers] generate-followups error:', err);
    res.status(500).json({ error: 'AI总结失败: ' + err.message });
  }
});

// ============ AI 需求总结 ============
router.post('/:id/ai-requirement', async (req, res, next) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return next('route');
    const id = parseInt(req.params.id);
    const customer = await prisma.customer.findUnique({ where: { id } });
    if (!customer) return res.status(404).json({ error: 'Customer not found' });

    const limit = Math.min(parseInt(req.body?.limit) || 50, 200);
    let messages = [];
    if (customer.jid) {
      messages = await prisma.wAMessage.findMany({
        where: { OR: [{ from: customer.jid }, { to: customer.jid }] },
        orderBy: { timestamp: 'desc' },
        take: limit,
      });
    }
    messages = messages.slice().reverse();

    if (messages.length === 0) {
      return res.status(400).json({ error: '暂无聊天记录，无法AI总结' });
    }

    const context = messages.map(m => {
      const who = (m.direction === 'outbound' || m.direction === 'outgoing') ? '我(业务员)' : '客户';
      const ts = m.timestamp ? new Date(m.timestamp).toISOString().slice(0, 16).replace('T', ' ') : '';
      return `[${ts}] ${who}: ${m.body || ''}`;
    }).join('\n');


    // 客户档案已知信息（AI已知事实，不要猜）
    const knownParts = [];
    if (customer.name) knownParts.push('客户名称：' + customer.name);
    if (customer.contactName && customer.contactName !== customer.name) knownParts.push('联系人：' + customer.contactName);
    if (customer.companyName || customer.company) knownParts.push('公司：' + (customer.companyName || customer.company));
    if (customer.country) knownParts.push('国家：' + customer.country);
    if (customer.title) knownParts.push('职位：' + customer.title);
    if (customer.website) knownParts.push('网站：' + customer.website);
    if (customer.phone) knownParts.push('电话：' + customer.phone);
    if (customer.email) knownParts.push('邮箱：' + customer.email);
    if (customer.tags) { try { const t=JSON.parse(customer.tags); if(Array.isArray(t)&&t.length) knownParts.push('标签：'+t.join('/')); } catch(_){} }
    const knownInfo = knownParts.length ? '【客户档案已知信息】\n' + knownParts.join('\n') + '\n' : '';

    const systemPrompt = `你是资深外贸B2B销售分析师。基于客户档案和WhatsApp聊天记录，生成一份结构化的客户需求分析报告。

【重要】必须输出严格的JSON，不要任何markdown标记、不要代码块包裹、不要解释文字。JSON结构如下：
{
  "title": "客户需求分析",
  "sections": [
    {
      "icon": "👤",
      "title": "客户基本信息",
      "type": "table",
      "rows": [{"label":"客户名称","value":"xxx"},{"label":"公司","value":"xxx"},{"label":"国家/地区","value":"xxx（推测/待确认）"},{"label":"职位/角色","value":"xxx"},{"label":"网站","value":"xxx"}]
    },
    {
      "icon": "📦",
      "title": "产品需求明细",
      "type": "table",
      "rows": [{"label":"产品类型","value":"xxx"},{"label":"规格参数","value":"xxx"},{"label":"包装要求","value":"xxx"},{"label":"认证/质量要求","value":"xxx"}]
    },
    {
      "icon": "📊",
      "title": "采购规模",
      "type": "table",
      "rows": [{"label":"采购量/频次","value":"xxx"},{"label":"订单性质","value":"xxx（试单/长期/一次性）"},{"label":"时间紧迫度","value":"xxx"}]
    },
    {
      "icon": "💼",
      "title": "商务条件",
      "type": "table",
      "rows": [{"label":"目标价/预算","value":"xxx"},{"label":"付款方式","value":"xxx"},{"label":"贸易条款","value":"xxx（FOB/CIF等）"},{"label":"起订量","value":"xxx"},{"label":"交期要求","value":"xxx"}]
    },
    {
      "icon": "🎯",
      "title": "客户画像",
      "type": "table",
      "rows": [{"label":"专业性/行业经验","value":"xxx"},{"label":"采购规模评级","value":"xxx"},{"label":"价格敏感度","value":"xxx"},{"label":"合作意向度","value":"高/中/低"},{"label":"谈判风格","value":"xxx"},{"label":"决策角色","value":"xxx"}]
    },
    {
      "icon": "⚠️",
      "title": "核心问题与风险",
      "type": "list",
      "items": ["风险点1：简短说明", "风险点2：简短说明"]
    },
    {
      "icon": "💡",
      "title": "建议跟进策略",
      "type": "list",
      "items": ["**策略1标题**：简短可执行说明", "**策略2标题**：简短可执行说明"]
    },
    {
      "icon": "❓",
      "title": "待确认事项",
      "type": "list",
      "items": ["问题1", "问题2"]
    }
  ]
}

填充规则：
1. 所有信息必须来自聊天记录或客户档案，严禁编造
2. 未提及的字段value直接填"未提及"，推测的标注"（推测）"，未知的填"待确认"
3. list板块items控制在2-5条，每条简短务实
4. 建议跟进策略每条用"**粗体关键词**：具体行动"格式（前端会自动加粗**之间**的文字）
5. 如果聊天全是测试消息/无实质业务，sections只保留一个板块：{"icon":"ℹ️","title":"提示","type":"text","content":"暂无可识别的采购需求，建议先与客户建立沟通。"}
6. 严格输出JSON，不要任何额外文字`;

    const userPrompt = knownInfo + `聊天记录（最近${messages.length}条，按时间顺序）：\n${context}\n\n请输出符合system要求的JSON：`;

    // 调用AI：先用Lite快速（豆包Lite），失败fallback到默认激活模型
    let raw;
    try {
      raw = await chatCompleteLite(
        [{role:'system', content:systemPrompt}, {role:'user', content:userPrompt}],
        { temperature: 0.3, maxTokens: 4000, timeout: 120000 }
      );
    } catch (e1) {
      console.warn('[Customers] ai-requirement chatCompleteLite failed, fallback to default:', e1.message);
      const defaultModel = await resolveModelId();
      raw = await chatComplete(defaultModel, systemPrompt, userPrompt, { temperature: 0.3, maxTokens: 4000, timeout: 120000 });
    }

    let rawText = (raw || '').trim();
    // 清理可能的markdown代码块包裹
    rawText = rawText.replace(/^```(?:json|markdown|md)?\s*/i, '').replace(/\s*```$/, '');
    // 【修复 2026-08-24】增强容错解析：LLM在json_object模式下常把JSON包成带转义引号的字符串
    let analysisData = null;
    // 1) 若整体是JSON字符串（以"开头），先解包
    if (rawText.charAt(0) === '"') {
      try {
        const unwrapped = JSON.parse(rawText);
        if (typeof unwrapped === 'string') rawText = unwrapped;
      } catch (_) {}
    }
    // 2) 反转义常见的 \" 和 \n（模型偶发把JSON作为字符串字面量返回）
    if (rawText.includes('\\"')) {
      rawText = rawText.replace(/\\"/g, '"').replace(/\\n/g, '\n').replace(/\\\\/g, '\\');
    }
    // 3) 找到第一个{和最后一个}之间的内容尝试解析
    const firstBrace = rawText.indexOf('{');
    const lastBrace = rawText.lastIndexOf('}');
    if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
      const candidate = rawText.slice(firstBrace, lastBrace + 1);
      const candidates = [candidate, candidate.replace(/,\s*([}\]])/g, '$1')];
      for (const cand of candidates) {
        try {
          const obj = JSON.parse(cand);
          if (obj && (Array.isArray(obj.sections) || Array.isArray(obj.replies))) { analysisData = obj; break; }
        } catch (_) {}
      }
    }
    // 4) 仍失败：把AI返回的（可能截断）内容清洗成可读文本，避免裸JSON字符
    if (!analysisData) {
      let cleaned = rawText
        .replace(/\\"/g, '"')
        .replace(/\\n/g, '\n')
        .slice(0, 2500);
      const lines = [];
      const pairRe = /"label"\s*:\s*"([^"]*)"\s*,\s*"value"\s*:\s*"([^"]*)"/g;
      let m; let found = false;
      while ((m = pairRe.exec(cleaned))) { found = true; lines.push(m[1] + '：' + m[2]); }
      if (!found) {
        const titleRe = /"title"\s*:\s*"([^"]*)"/g;
        while ((m = titleRe.exec(cleaned))) { found = true; lines.push(m[1]); }
      }
      if (found) {
        cleaned = lines.join('\n');
      } else {
        cleaned = cleaned.replace(/[{}"[\]]/g, '').replace(/,\s*/g, '\n').replace(/\n{2,}/g, '\n').trim();
      }
      analysisData = {
        title: '客户需求分析',
        sections: [{ icon: '📝', title: 'AI分析', type: 'text', content: cleaned || rawText.slice(0, 1500) }]
      };
    }
    // 存一份JSON字符串到summary字段（前端解析渲染）
    const summaryJson = JSON.stringify(analysisData);

    await prisma.customer.update({
      where: { id },
      data: {
        requirementSummary: summaryJson,
        requirementSource: 'ai',
        requirementUpdatedAt: new Date(),
      },
    });

    res.json({ summary: summaryJson, structured: true, raw });
  } catch (err) {
    console.error('[Customers] ai-requirement error:', err);
    res.status(500).json({ error: 'AI需求总结失败: ' + err.message });
  }
});



// POST /:id/ai-document — AI-generate a PI or Quotation for a customer
router.post('/:id/ai-document', async (req, res, next) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return next('route');
    const id = parseInt(req.params.id);
    const type = (req.body?.type || 'PI').toUpperCase();
    if (!['PI', 'QUOTATION'].includes(type)) return res.status(400).json({ error: 'type must be PI or QUOTATION' });
    const limit = Math.min(parseInt(req.body?.limit) || 50, 200);

    const customer = await prisma.customer.findUnique({ where: { id } });
    if (!customer) return res.status(404).json({ error: 'Customer not found' });

    // Collect context: customer info + requirement fields
    const custInfo = [
      customer.companyName ? `公司: ${customer.companyName}` : '',
      customer.contactName ? `联系人: ${customer.contactName}` : '',
      customer.country ? `国家: ${customer.country}` : '',
      customer.email ? `邮箱: ${customer.email}` : '',
      customer.phone ? `电话: ${customer.phone}` : '',
      customer.address ? `地址: ${customer.address}` : '',
    ].filter(Boolean).join('\n');

    const reqInfo = [];
    if (customer.requirementProducts) reqInfo.push(`意向产品: ${customer.requirementProducts}`);
    if (customer.requirementQuantity) reqInfo.push(`数量: ${customer.requirementQuantity}`);
    if (customer.requirementBudget) reqInfo.push(`预算/目标价: ${customer.requirementBudget}`);
    if (customer.requirementDelivery) reqInfo.push(`交期: ${customer.requirementDelivery}`);
    if (customer.requirementSummary) reqInfo.push(`需求详情: ${customer.requirementSummary}`);

    // Load recent messages
    let messages = [];
    if (customer.jid) {
      messages = await prisma.wAMessage.findMany({
        where: { OR: [{ from: customer.jid }, { to: customer.jid }] },
        orderBy: { timestamp: 'desc' }, take: limit,
      });
    }
    messages = messages.slice().reverse();

    const chatContext = messages.map(m => {
      const who = (m.direction === 'outbound' || m.direction === 'outgoing') ? '我(业务员)' : '客户';
      const ts = m.timestamp ? new Date(m.timestamp).toISOString().slice(0,16).replace('T',' ') : '';
      return `[${ts}] ${who}: ${m.body || ''}`;
    }).join('\n');

    const systemPrompt = `你是外贸B2B单证助手，根据客户档案和WhatsApp聊天记录生成PI或报价单数据。
请严格输出JSON（不要markdown代码块，不要其他文字），字段：
{
  "buyerName": "买方公司名",
  "buyerAddress": "买方地址（或国家）",
  "buyerContact": "联系人",
  "buyerEmail": "邮箱",
  "buyerPhone": "电话",
  "items": [{"productName":"产品名称","model":"型号","spec":"规格","quantity":数字,"unit":"单位(㎡/pcs/set)","unitPrice":数字,"remark":""}],
  "currency": "USD",
  "tradeTerm": "FOB/CIF/EXW等",
  "paymentTerm": "付款方式",
  "portOfLoading": "装运港",
  "remarks": "备注"
}
- 产品明细items要合理：从聊天和需求里识别客户询问的产品、规格、数量、价格区间，金额=数量×单价
- 如果客户没明确数量/价格，按该产品常见起订量和合理市场价填写（玻璃产品用玻璃行业常见价格）
- 金额字段必须是数字，不要带货币符号
- 输出严格JSON，不要解释`;

    const userPrompt = `【客户信息】\n${custInfo}\n\n【需求总结】\n${reqInfo.join('\n') || '(无)'}\n\n【最近聊天（${messages.length}条）】\n${chatContext.slice(-6000)}\n\n请生成一份${type === 'PI' ? 'PI (Proforma Invoice)' : 'Quotation (报价单)'}的JSON：`;

    let raw;
    try {
      raw = await chatCompleteLite(
        [{role:'system', content:systemPrompt}, {role:'user', content:userPrompt}],
        { temperature: 0.3, maxTokens: 1800, timeout: 60000 }
      );
    } catch (e) {
      console.warn('[AI-Doc] chatCompleteLite failed, fallback:', e.message);
      const defaultModel = await resolveModelId();
      raw = await chatComplete(defaultModel, systemPrompt, userPrompt, { temperature: 0.3, maxTokens: 1800, timeout: 60000 });
    }

    // Parse JSON
    let parsed;
    try {
      const m = raw.match(/\{[\s\S]*\}/);
      if (!m) throw new Error('no json in response');
      parsed = JSON.parse(m[0]);
    } catch (e) {
      console.error('[AI-Doc] JSON parse error:', e.message);
      return res.status(500).json({ error: 'AI返回格式异常，请重试' });
    }

    const items = (parsed.items || []).map((it, idx) => ({
      sortOrder: idx,
      productName: it.productName || it.description || 'Product',
      model: it.model || '',
      spec: it.spec || '',
      quantity: Number(it.quantity) || 0,
      unit: it.unit || 'pcs',
      unitPrice: Number(it.unitPrice) || 0,
      amount: Math.round((Number(it.quantity)||0) * (Number(it.unitPrice)||0) * 100)/100,
      remark: it.remark || '',
    }));

    const total = Math.round(items.reduce((s,i)=>s+i.amount,0) * 100)/100;
    if (total <= 0 || items.length === 0) {
      return res.status(400).json({ error: 'AI未能识别到有效产品明细，请先完善需求总结或在编辑器手动添加' });
    }

    // Generate doc number
    const prefix = type === 'PI' ? 'PI' : 'QT';
    const today = new Date();
    const datePrefix = `${today.getFullYear()}${String(today.getMonth()+1).padStart(2,'0')}${String(today.getDate()).padStart(2,'0')}`;
    const startWith = `${prefix}${datePrefix}`;
    const lastDoc = await prisma.document.findFirst({
      where: { docNumber: { startsWith: startWith } },
      orderBy: { docNumber: 'desc' },
      select: { docNumber: true },
    });
    let seq = 1;
    if (lastDoc?.docNumber) {
      const mm = lastDoc.docNumber.match(new RegExp('^' + startWith.replace(/[-]/g,'\\-') + '(\\d+)$'));
      if (mm) seq = parseInt(mm[1]) + 1;
    }
    const docNumber = `${startWith}${String(seq).padStart(3,'0')}`;

    const issueDate = new Date();
    let validUntil = null;
    if (type === 'QUOTATION') { validUntil = new Date(issueDate); validUntil.setDate(validUntil.getDate()+30); }

    const currency = parsed.currency || 'USD';
    const sellerInfo = loadSellerInfo();

    const buyerInfo = {
      companyName: parsed.buyerName || customer.companyName || customer.name || '',
      address: parsed.buyerAddress || customer.address || '',
      contactName: parsed.buyerContact || customer.contactName || '',
      phone: parsed.buyerPhone || customer.phone || '',
      email: parsed.buyerEmail || customer.email || '',
      country: customer.country || '',
    };

    const created = await prisma.document.create({
      data: {
        userId: req.userId || 1,
        customerId: id,
        customerJid: customer.jid || null,
        type,
        docNumber,
        title: type === 'PI' ? 'Proforma Invoice' : 'Quotation',
        issueDate,
        validUntil,
        currency,
        totalAmount: total,
        amountInWords: numberToWords(total, currency),
        status: 'DRAFT',
        sellerInfo,
        buyerInfo,
        tradeTerm: parsed.tradeTerm || 'FOB',
        paymentTerm: parsed.paymentTerm || 'T/T 30% deposit, 70% before shipment',
        portOfLoading: parsed.portOfLoading || 'Shenzhen, China',
        portOfDest: '',
        shipDate: '',
        remarks: parsed.remarks || (type === 'PI'
          ? 'Please confirm this PI and arrange the deposit.'
          : 'This quotation is valid for 30 days from the date of issue.'),
        items: { create: items },
      },
      include: { items: true },
    });

    res.status(201).json({ docId: created.id, document: { id: created.id, docNumber: created.docNumber, type, totalAmount: total } });
  } catch (err) {
    console.error('[Customers] ai-document error:', err);
    res.status(500).json({ error: 'AI生成单证失败: ' + err.message });
  }
});

// Legacy by-phone
router.get('/by-phone/:phone', async (req, res) => {
  try {
    const customer = await prisma.customer.findFirst({ where: { phone: req.params.phone } });
    if (!customer) return res.status(404).json({ error: 'Customer not found' });
    res.json(serializeCustomer(customer));
  } catch (err) {
    console.error('[Customers] By-phone error:', err);
    res.status(500).json({ error: 'Failed to find customer' });
  }
});


// ─── 客户背调 bg-check 路由 ───

// 从JID里提取纯号码
function extractPhoneFromJid(jid) {
  if (!jid) return '';
  const s = String(jid).split('@')[0];
  return s.replace(/[^0-9]/g, '');
}

// 国家代码→国家名（常见外贸市场，供LLM兜底）
const CC_COUNTRY = {
  '1': '美国/加拿大', '12': '加拿大', '18': '多米尼加',
  '44': '英国', '61': '澳大利亚', '64': '新西兰', '81': '日本', '82': '韩国',
  '86': '中国大陆', '852': '中国香港', '886': '中国台湾', '65': '新加坡', '60': '马来西亚',
  '66': '泰国', '84': '越南', '62': '印度尼西亚', '63': '菲律宾', '91': '印度',
  '92': '巴基斯坦', '971': '阿联酋', '966': '沙特阿拉伯', '972': '以色列',
  '90': '土耳其', '7': '俄罗斯', '49': '德国', '33': '法国', '39': '意大利',
  '34': '西班牙', '351': '葡萄牙', '31': '荷兰', '32': '比利时', '46': '瑞典',
  '47': '挪威', '45': '丹麦', '48': '波兰', '41': '瑞士', '43': '奥地利',
  '30': '希腊', '36': '匈牙利', '353': '爱尔兰', '380': '乌克兰',
  '52': '墨西哥', '55': '巴西', '54': '阿根廷', '56': '智利', '57': '哥伦比亚',
  '51': '秘鲁', '507': '巴拿马', '503': '萨尔瓦多', '502': '危地马拉',
  '20': '埃及', '27': '南非', '234': '尼日利亚', '254': '肯尼亚',
  '212': '摩洛哥', '213': '阿尔及利亚', '216': '突尼斯',
};

function guessCountryByPhone(phone) {
  if (!phone) return '';
  const p = String(phone);
  // 3位优先
  for (let len = 3; len >= 1; len--) {
    const cc = p.substring(0, len);
    if (CC_COUNTRY[cc]) return CC_COUNTRY[cc];
  }
  return '';
}

// DuckDuckGo HTML 搜索（10秒超时，无新依赖，原生https）
function duckSearch(query, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    const q = encodeURIComponent(query);
    const url = `https://html.duckduckgo.com/html/?q=${q}`;
    const req = https.get(url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36',
        'Accept': 'text/html,application/xhtml+xml',
        'Accept-Language': 'en-US,en;q=0.9',
      },
      timeout: timeoutMs,
    }, (res) => {
      // 处理重定向
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        res.resume();
        // 简化：忽略重定向，直接返回空
        return resolve([]);
      }
      let data = '';
      res.setEncoding('utf8');
      res.on('data', (chunk) => { data += chunk; });
      res.on('end', () => {
        try {
          const results = [];
          // 提取 result__a title 和 result__snippet
          const resultRegex = /<a[^>]*class="result__a"[^>]*href="([^"]*)"[^>]*>([\s\S]*?)<\/a>[\s\S]*?<a[^>]*class="result__snippet"[^>]*>([\s\S]*?)<\/a>/g;
          let m;
          while ((m = resultRegex.exec(data)) !== null && results.length < 5) {
            const href = m[1];
            const title = m[2].replace(/<[^>]+>/g, '').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#x27;/g, "'").replace(/&lt;/g, '<').replace(/&gt;/g, '>').trim();
            const snippet = m[3].replace(/<[^>]+>/g, '').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#x27;/g, "'").replace(/&lt;/g, '<').replace(/&gt;/g, '>').trim();
            if (title) results.push({ title, snippet, href });
          }
          resolve(results);
        } catch (e) {
          resolve([]);
        }
      });
    });
    req.on('error', (e) => reject(e));
    req.on('timeout', () => { req.destroy(); reject(new Error('timeout')); });
  });
}

async function performWebSearch(customer) {
  const phone = extractPhoneFromJid(customer.jid || customer.phone);
  const companyName = customer.companyName || '';
  const contactName = customer.contactName || '';
  const country = customer.country || guessCountryByPhone(phone);

  const queries = [];
  if (phone) {
    queries.push(`${phone} whatsapp`);
    queries.push(`${phone} company`);
  }
  if (companyName) {
    queries.push(`"${companyName}" official website`);
    if (country) queries.push(`"${companyName}" ${country} business`);
  }
  if (contactName && companyName) {
    queries.push(`"${contactName}" "${companyName}" linkedin`);
  }

  const allResults = [];
  let failedQueryCount = 0;
  for (let i = 0; i < queries.length; i++) {
    const q = queries[i];
    try {
      const rs = await duckSearch(q, 10000);
      if (rs.length) {
        allResults.push({ query: q, results: rs });
      }
    } catch (e) {
      failedQueryCount++;
      console.warn('[bg-check] search failed for:', q, e.message);
    }
    // Add random delay between searches to avoid DDG rate limiting (3-5 seconds)
    if (i < queries.length - 1) {
      const delay = 3000 + Math.random() * 2000;
      await new Promise(r => setTimeout(r, delay));
    }
  }
  return { allResults, failedQueryCount, totalQueries: queries.length };
}


// === GET /by-jid/:jid/bg-check — 获取背调状态 ===
router.get('/by-jid/:jid/bg-check', async (req, res) => {
  try {
    const jid = decodeJid(req.params.jid);
    const userId = req.userId;
    const customer = await findCustomerByJid(prisma, userId, jid);
    if (!customer) {
      return res.json({ hasReport: false });
    }
    let bgCheck = null;
    try {
      bgCheck = await prisma.customerBackgroundCheck.findUnique({ where: { contactId: customer.id } });
    } catch (e) { /* table may not exist yet */ }

    if (!customer.bgReport && !bgCheck) {
      return res.json({ hasReport: false });
    }

    const result = {
      hasReport: true,
      rating: bgCheck?.riskLevel || null,
      summary: bgCheck?.notes || customer.bgReport || '',
      updatedAt: (bgCheck?.updatedAt || customer.bgUpdatedAt)?.toISOString?.() || null,
      dimensions: [],
      sources: [],
      companyName: bgCheck?.companyName || customer.companyName || null,
      country: bgCheck?.country || customer.country || null,
      industry: bgCheck?.industry || customer.industry || null,
      companySize: bgCheck?.companySize || null,
    };
    if (bgCheck?.details) {
      try {
        const parsed = JSON.parse(bgCheck.details);
        if (Array.isArray(parsed.dimensions)) result.dimensions = parsed.dimensions;
        if (Array.isArray(parsed.sources)) result.sources = parsed.sources;
        if (parsed.score !== undefined) result.score = parsed.score;
        if (Array.isArray(parsed.chapters)) result.chapters = parsed.chapters;
        if (parsed.executiveSummary) result.executiveSummary = parsed.executiveSummary;
        if (parsed.recommendation) result.recommendation = parsed.recommendation;
      } catch {}
    }
    res.json(result);
  } catch (err) {
    console.error('[bg-check GET] error:', err);
    res.status(500).json({ error: 'Failed to get bg check status' });
  }
});

router.post('/by-jid/:jid/bg-check', async (req, res) => {
  try {
    const jid = decodeJid(req.params.jid);
    const userId = req.userId;

    // 查找客户，找不到就用jid临时构造一个最小客户（jid里至少有号码）
    let customer = await findCustomerByJid(prisma, userId, jid);
    const isNew = !customer;
    if (!customer) {
      const phone = extractPhoneFromJid(jid);
      customer = {
        jid,
        userId,
        phone,
        companyName: null,
        contactName: null,
        country: null,
        title: null,
        email: null,
        website: null,
        notes: null,
        industry: null,
      };
    }

    const phone = extractPhoneFromJid(customer.jid || customer.phone);
    const guessedCountry = guessCountryByPhone(phone);
    const country = customer.country || guessedCountry;

    // 联网搜索
    console.log('[bg-check] starting search for', jid);
    let searchInfo = { allResults: [], failedQueryCount: 0, totalQueries: 0 };
    let searchFailed = false;
    try {
      searchInfo = await performWebSearch({ ...customer, country });
    } catch (e) {
      console.warn('[bg-check] overall search failed:', e.message);
      searchFailed = true;
    }
    // 没结果也算失败走LLM兜底提示
    const noHit = searchInfo.allResults.length === 0;

    // 拼搜索摘要文本
    let searchText = '';
    if (!searchFailed && !noHit) {
      for (const group of searchInfo.allResults) {
        searchText += `\n[搜索词: ${group.query}]\n`;
        group.results.forEach((r, i) => {
          searchText += `${i + 1}. ${r.title}\n   ${r.snippet}\n`;
        });
      }
    } else {
      searchText = '(联网搜索未获取到公开结果，请直接基于已有信息生成初步背调分析)';
    }

    const knownInfo = [
      `WhatsApp号码(JID): ${jid}`,
      `纯号码: ${phone || '未知'}`,
      `联系人名: ${customer.contactName || '未知'}`,
      `公司名: ${customer.companyName || '未知'}`,
      `职位: ${customer.title || '未知'}`,
      `国家/地区字段: ${customer.country || '未填写'}（按号码国际区号推断: ${guessedCountry || '无法推断'}）`,
      `邮箱: ${customer.email || '未知'}`,
      `网站: ${customer.website || '未知'}`,
      `行业: ${customer.industry || '未知'}`,
      `来源: ${customer.source || '未知'}`,
      `客户等级: ${customer.customerLevel || 'C'}`,
    ].join('\n');

    // LLM提示词
    const systemPrompt = `你是资深外贸情报分析师，负责为WhatsApp上的海外客户生成背调报告。
输出要求：
1. 必须严格使用纯文本，禁止使用任何markdown格式符号（禁止#、*、-、|、\`等），分区用emoji+标题，段落之间用空行分隔。
2. 报告语言为简体中文。
3. 如果联网搜索有结果，请结合搜索结果给出具体内容；如果搜索没结果，直接基于已知字段+号码区号做合理推断，不要胡编具体公司名/网址等未验证信息，未验证的点要标注"待核实"。
4. 输出必须包含并严格按以下7个分区顺序输出（每个分区用emoji开头+标题+换行，再写正文）：

👤 客户基本信息
（整理已知字段：号码、姓名、职位、公司、国家、邮箱、网站、来源、客户等级等；号码至少一定有）

🏢 公司背景
（如果有公司名，结合搜索结果给出公司业务、官网、行业、规模等可能信息；无公司名则提示"尚未获知公司名，建议优先询问"）

🌍 国家/地区市场概况
（基于country字段或号码国际区号推断客户所在市场，简述该市场的贸易特征、商业习惯、对华贸易情况）

📱 社交足迹
（基于搜索结果描述该号码在网上的痕迹，如果没搜到，写"暂未检索到该号码的公开社交/商业痕迹"并说明可能原因）

⚠️ 风险提示
（列出可疑点或注意事项：如新客户、无公司信息、号码归属地异常、高风险地区等，最多5条，简明扼要）

💡 跟进建议
（给出3-5条具体可执行的跟进策略，结合客户已知信息、国家市场特点）

❓ 信息缺口
（列出目前还缺失的关键信息，以及建议询问客户的问题方向——这一部分无论搜索成功与否都必须输出，至少列出3条）

重要：
- 严禁编造未被已知信息或搜索结果支持的具体公司名、网址、人名。
- 不要在正文里出现JSON或代码块。
- 每个分区之间留一个空行。`;

    const searchNote = searchFailed
      ? '⚠️ 注意：联网搜索全部失败/超时，以下报告完全基于已有客户信息+号码国家区号生成，请作为初步参考，missingInfo里需注明"联网搜索失败，以下为基于已有信息的初步分析"。'
      : (noHit ? '⚠️ 注意：联网搜索未返回公开结果，请基于已有信息+国家区号做合理推断。' : '');

    const userPrompt = `${searchNote}

=== 已知客户信息 ===
${knownInfo}

=== 联网搜索结果（如有）===
${searchText}

请开始生成背调报告。`;

    // 用defaultModel（不是DOUBAO_LITE）
    const defaultModel = await resolveModelId();
    let rawReport = '';
    try {
      rawReport = await chatComplete(defaultModel, systemPrompt, userPrompt, { temperature: 0.4, maxTokens: 2500, timeout: 90000 });
    } catch (e) {
      console.error('[bg-check] LLM primary failed, retry once:', e.message);
      try {
        rawReport = await chatComplete(defaultModel, systemPrompt, userPrompt, { temperature: 0.4, maxTokens: 2500, timeout: 90000 });
      } catch (e2) {
        console.error('[bg-check] LLM retry failed:', e2.message);
        return res.status(500).json({ error: 'AI生成背调失败: ' + e2.message });
      }
    }
    const report = (rawReport || '').trim();
    // 空报告不落库，直接报错提示重试
    if (!report) {
      return res.status(500).json({ error: 'AI生成背调失败：返回内容为空，请重试' });
    }

    // missingInfo 判定（基于已知字段）
    const missingInfo = [];
    if (!customer.companyName) missingInfo.push('公司名');
    if (!customer.contactName) missingInfo.push('联系人全名');
    if (!customer.title) missingInfo.push('联系人职位');
    if (!customer.email) missingInfo.push('邮箱');
    if (!customer.website) missingInfo.push('公司网站');
    if (!customer.country && !guessedCountry) missingInfo.push('所在国家/地区');
    if (!customer.industry) missingInfo.push('所属行业');
    if (!customer.address) missingInfo.push('公司地址');
    if (searchFailed || noHit) missingInfo.push('联网搜索失败，以下为基于已有信息的初步分析');

    // 生成询问话术 askReply：用客户语言（按国家推断，默认英文）生成询问缺失信息的话术+中文翻译
    let askReply = '';
    try {
      const askSys = `你是外贸业务员助手。现在我对一个WhatsApp客户的背景信息不全，请根据目前缺失的信息字段，生成一段自然、不生硬的询问话术，一次性礼貌地把最关键的缺失信息问出来（不要像问卷，要像自然聊天）。
输出格式严格如下两行：
[外文本土化语言，根据客户国家/号码区号推断：英文默认；日本=日语；韩国=韩语；西语/葡语/法语/德语/阿语/俄语区用对应语言]
[中文翻译]
不要输出任何其他字符，不要markdown，不要标签。`;
      const askUser = `缺失信息：${missingInfo.filter(x => !x.includes('联网搜索')).join('、') || '请帮我自然地进一步了解客户背景'}
客户所在国家/地区：${country || '未知（默认英文）'}
已知信息摘要：${knownInfo}
请生成话术。`;
      askReply = await chatComplete(defaultModel, askSys, askUser, { temperature: 0.8, maxTokens: 500, timeout: 60000 });
    } catch (e) {
      console.warn('[bg-check] askReply gen failed, fallback:', e.message);
      askReply = `By the way, could you kindly share a bit more about your company and your role? That would help us serve you better.\n顺便问一下，能简单介绍一下贵公司和您的职位吗？这样方便我们更好地为您服务。`;
    }

    // 持久化到Customer表
    const now = new Date();
    try {
      if (isNew) {
        const created = await prisma.customer.create({
          data: {
            userId, jid, name: customer.contactName || '',
            companyName: customer.companyName, contactName: customer.contactName,
            country: country, phone: phone, email: customer.email,
            website: customer.website, source: customer.source || 'whatsapp',
            customerLevel: 'C', bgReport: report, bgUpdatedAt: now,
            isBusiness: true, // L2: 用户主动发起背调建档 = 明确动作，直接视为业务客户
          },
        });
        try {
          await prisma.customerBackgroundCheck.upsert({
            where: { contactId: created.id },
            update: { companyName: customer.companyName || null, country: country || null, industry: customer.industry || null, details: JSON.stringify({ textReport: report }), riskLevel: null, notes: report, source: 'llm' },
            create: { contactId: created.id, companyName: customer.companyName || null, country: country || null, industry: customer.industry || null, details: JSON.stringify({ textReport: report }), riskLevel: null, notes: report, source: 'llm' }
          });
        } catch (bgcErr) { console.warn('[bg-check] save CBC failed:', bgcErr.message); }
      } else {
        await prisma.customer.update({ where: { id: customer.id }, data: { bgReport: report, bgUpdatedAt: now } });
        try {
          await prisma.customerBackgroundCheck.upsert({
            where: { contactId: customer.id },
            update: { companyName: customer.companyName || null, country: country || null, industry: customer.industry || null, details: JSON.stringify({ textReport: report }), riskLevel: null, notes: report, source: 'llm' },
            create: { contactId: customer.id, companyName: customer.companyName || null, country: country || null, industry: customer.industry || null, details: JSON.stringify({ textReport: report }), riskLevel: null, notes: report, source: 'llm' }
          });
        } catch (bgcErr) { console.warn('[bg-check] save CBC failed:', bgcErr.message); }
      }
    } catch (dbErr) {
      console.warn('[bg-check] save bgReport to DB failed:', dbErr.message);
    }

    res.json({
      report,
      missingInfo,
      askReply: (askReply || '').trim(),
      bgUpdatedAt: now.toISOString(),
      isNew,
      searchUsed: !(searchFailed || noHit),
    });
  } catch (err) {
    console.error('[Customers] bg-check error:', err);
    res.status(500).json({ error: '背调失败: ' + err.message });
  }
});

export default router;

// ===== AI 自动补全客户信息（入站消息后异步调用） =====
// 规则：只填空字段，不覆盖已有值；24h冷却；不阻塞主流程
const AUTO_FILL_FIELDS = ['companyName','contactName','country','title','email','website','address','industry','requirementProducts'];

async function extractCustomerInfoFromChat(jid, limit = 20) {
  const messages = await prisma.wAMessage.findMany({
    where: { OR: [{ from: jid }, { to: jid }] },
    orderBy: { timestamp: 'desc' },
    take: limit,
  });
  if (!messages.length) return null;
  const context = messages.slice().reverse().map(m => {
    const who = (m.direction === 'outbound' || m.direction === 'outgoing') ? '我(业务员)' : '客户';
    const ts = m.timestamp ? new Date(m.timestamp).toISOString().slice(0, 16).replace('T', ' ') : '';
    const body = (m.body || '').replace(/```[\s\S]*?```/g, '[图片]').slice(0, 500);
    return `[${ts}] ${who}: ${body}`;
  }).join('\n');
  if (!context.trim()) return null;

  const modelId = await resolveModelId();
  const systemPrompt = `你是一个外贸客户信息抽取助手。根据以下WhatsApp聊天记录，抽取客户关键信息。

## 抽取字段
- companyName: 客户公司名（英文/原文），没有则null
- contactName: 客户联系人名/称呼，没有则null
- country: 客户国家/地区，没有则null
- title: 客户职位/头衔，没有则null
- email: 邮箱地址，没有则null
- website: 网址，没有则null
- address: 客户地址/城市，没有则null
- industry: 行业，没有则null
- requirementProducts: 客户提到的采购产品/意向产品（英文关键词，逗号分隔），没有则null

要求：
- 严格只输出JSON，不要markdown代码块，不要其他文字
- 不确定的字段设null，不要瞎猜
- 邮箱/网址/国家要精确，宁可null不要猜测
- 产品字段只记录客户明确提到要买的产品，不要把我方推销的产品算进去
- 格式：{"companyName":"...","contactName":"...","country":"...","title":"...","email":"...","website":"...","address":"...","industry":"...","requirementProducts":"..."}`;

  const userPrompt = `聊天记录（最近${messages.length}条）：\n${context}\n\n请抽取：`;
  const raw = await chatComplete(modelId, systemPrompt, userPrompt, { temperature: 0.2, maxTokens: 600 });
  let parsed = null;
  try {
    const m = raw.match(/\{[\s\S]*\}/);
    if (m) parsed = JSON.parse(m[0]);
  } catch (e) {
    console.error('[autoFill] JSON parse error:', e.message, raw);
    return null;
  }
  if (!parsed || typeof parsed !== 'object') return null;
  const result = {};
  for (const f of AUTO_FILL_FIELDS) {
    const v = parsed[f];
    if (v && typeof v === 'string' && v.trim() && v.trim().toLowerCase() !== 'null' && v.trim() !== '未知') {
      result[f] = v.trim();
    }
  }
  return Object.keys(result).length ? result : null;
}

/**
 * 异步自动填充客户空字段。fire-and-forget，不抛异常。
 * @param {string} jid - 客户jid
 * @param {number} [userId=1]
 * @returns {Promise<{filled: string[], customerId: number}|null>}
 */
// ============ 销售漏斗 API ============
const DEAL_STAGES = ['new','qualified','quoting','negotiating','won','completed','lost'];
const STAGE_META = {
  new:        { label:'新询盘', color:'#6b7280', icon:'🆕' },
  qualified:  { label:'需求确认', color:'#3b82f6', icon:'🔍' },
  quoting:    { label:'报价中',   color:'#f59e0b', icon:'💰' },
  negotiating:{ label:'谈判/寄样',color:'#8b5cf6', icon:'🤝' },
  won:        { label:'成交待付款',color:'#10b981', icon:'🎉' },
  completed:  { label:'已完成',   color:'#059669', icon:'✅' },
  lost:       { label:'已流失',   color:'#ef4444', icon:'❌' },
};

// GET /pipeline - 按阶段分组返回客户（注意：必须在 /:id 之前，Express按顺序匹配）
router.get('/pipeline', async (req, res) => {
  try {
    const userId = req.userId;
    const customers = await prisma.customer.findMany({
      where: { userId },
      orderBy: { updatedAt: 'desc' },
      include: { _count: { select: { followUps: true } } },
    });
    const stages = {};
    for (const st of DEAL_STAGES) {
      stages[st] = { key: st, ...STAGE_META[st], items: [], count: 0, value: 0 };
    }
    for (const c of customers) {
      const st = DEAL_STAGES.includes(c.dealStage) ? c.dealStage : 'new';
      stages[st].items.push({
        id: c.id, name: c.name, companyName: c.companyName, country: c.country,
        customerLevel: c.customerLevel, dealValue: c.dealValue || 0,
        dealStage: st, dealStageAt: c.dealStageAt, lastContactAt: c.lastContactAt,
        _followUpCount: c._count?.followUps || 0,
      });
      stages[st].count++;
      stages[st].value += c.dealValue || 0;
    }
    const totalValue = Object.values(stages).reduce((a,s)=>a+s.value,0);
    res.json({ stages: DEAL_STAGES.map(k=>stages[k]), totalValue, total: customers.length });
  } catch (err) {
    console.error('[Pipeline] error:', err);
    res.status(500).json({ error: 'Failed to load pipeline' });
  }
});

// PATCH /:id/stage - 更新客户阶段（必须在 /:id 路由之前）
router.patch('/:id/stage', async (req, res) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return res.status(400).json({ error: 'Invalid id' });
    const id = parseInt(req.params.id);
    const { stage, note, dealValue } = req.body;
    if (!DEAL_STAGES.includes(stage)) return res.status(400).json({ error: 'Invalid stage' });
    const customer = await prisma.customer.findUnique({ where: { id } });
    if (!customer) return res.status(404).json({ error: 'Customer not found' });
    const oldStage = customer.dealStage || 'new';
    const data = { dealStage: stage, dealStageAt: new Date() };
    if (typeof dealValue === 'number' && dealValue >= 0) data.dealValue = dealValue;
    const updated = await prisma.customer.update({ where: { id }, data });
    // 写跟进记录
    const meta = STAGE_META[stage];
    const oldMeta = STAGE_META[oldStage] || { label: oldStage };
    let content = `📋 阶段变更：${oldMeta.label} → ${meta.label}`;
    if (note) content += `\n📝 备注：${note}`;
    await prisma.customerFollowUp.create({
      data: { customerId: id, content, source: 'stage_change', metadata: JSON.stringify({ from: oldStage, to: stage }) },
    });
    res.json({ ok: true, customer: serializeCustomer(updated) });
  } catch (err) {
    console.error('[Pipeline] stage update error:', err);
    res.status(500).json({ error: 'Failed to update stage' });
  }
});

// GET /:id/stage-history - 获取阶段变更历史
router.get('/:id/stage-history', async (req, res) => {
  try {
    if (!/^\d+$/.test(req.params.id)) return res.status(400).json({ error: 'Invalid id' });
    const id = parseInt(req.params.id);
    const records = await prisma.customerFollowUp.findMany({
      where: { customerId: id, source: 'stage_change' },
      orderBy: { createdAt: 'desc' },
      take: 20,
    });
    res.json(records.map(r => {
      let meta = {};
      try { meta = JSON.parse(r.metadata || '{}'); } catch {}
      return { id: r.id, from: meta.from, to: meta.to, note: r.content, createdAt: r.createdAt };
    }));
  } catch (err) {
    console.error('[Pipeline] history error:', err);
    res.status(500).json({ error: 'Failed to load history' });
  }
});

export async function autoFillCustomer(jid, userId = 1) {
  try {
    if (!jid) return null;
    let cust = await findCustomerByJid(prisma, userId, jid);
    if (!cust) {
      // fallback：按 jid 查（兼容 userId 传错/多租户历史数据），取最新
      const variants = normalizeJid(jid);
      if (variants.length) cust = await prisma.customer.findFirst({ where: { jid: { in: variants } }, orderBy: { updatedAt: 'desc' } });
    }
    if (!cust) return null;
    // 24h冷却：aiExtractedAt 24小时内跳过
    if (cust.aiExtractedAt && (Date.now() - new Date(cust.aiExtractedAt).getTime()) < 24*3600*1000) {
      return null;
    }
    const extracted = await extractCustomerInfoFromChat(jid, 50);
    if (!extracted) {
      await prisma.customer.update({ where: { id: cust.id }, data: { aiExtractedAt: new Date() } });
      return null;
    }
    // 只填空字段
    const updateData = {};
    const newlyFilled = [];
    let existingFields = {};
    try { existingFields = cust.aiAutoFields ? JSON.parse(cust.aiAutoFields) : []; } catch { existingFields = []; }
    if (!Array.isArray(existingFields)) existingFields = [];

    for (const f of AUTO_FILL_FIELDS) {
      const current = cust[f];
      const isEmpty = !current || (typeof current === 'string' && !current.trim());
      if (isEmpty && extracted[f]) {
        updateData[f] = extracted[f];
        if (!existingFields.includes(f)) existingFields.push(f);
        newlyFilled.push(f);
      }
    }
    // 城市同步：AI 抽取的 address 同时填充 city（前端"城市/地区"用 city 字段）
    if (!updateData.city && extracted.address && !cust.city) {
      updateData.city = extracted.address;
      if (!existingFields.includes('city')) existingFields.push('city');
      if (!newlyFilled.includes('city')) newlyFilled.push('city');
    }
    if (!newlyFilled.length) {
      await prisma.customer.update({ where: { id: cust.id }, data: { aiExtractedAt: new Date() } });
      return null;
    }
    updateData.aiAutoFields = JSON.stringify(existingFields);
    updateData.aiExtractedAt = new Date();
    await prisma.customer.update({ where: { id: cust.id }, data: updateData });
    console.log(`[autoFill] jid=${jid} filled fields: ${newlyFilled.join(',')}`);
    return { filled: newlyFilled, customerId: cust.id };
  } catch (err) {
    console.error('[autoFill] error:', err.message);
    return null;
  }
}

// ===== 自动背调（入站消息后异步调用） =====
// 规则：7天冷却；至少要有公司名或联系人名；不阻塞主流程
export async function autoBackgroundCheck(jid, userId = 1, io = null) {
  try {
    if (!jid) return { skipped: true, reason: 'no jid' };
    let cust = await findCustomerByJid(prisma, userId, jid);
    if (!cust) {
      // fallback：按 jid 查（兼容 userId 传错/多租户历史数据），取最新
      const variants = normalizeJid(jid);
      if (variants.length) cust = await prisma.customer.findFirst({ where: { jid: { in: variants } }, orderBy: { updatedAt: 'desc' } });
    }
    if (!cust) return { skipped: true, reason: 'customer not found' };

    // 7天冷却
    if (cust.bgUpdatedAt) {
      const elapsed = Date.now() - new Date(cust.bgUpdatedAt).getTime();
      if (elapsed < 7 * 24 * 3600 * 1000) {
        return { skipped: true, reason: 'within 7-day cooldown' };
      }
    }

    // 至少要有公司名或联系人名
    const hasCompany = cust.companyName && cust.companyName.trim();
    const hasContactName = cust.contactName && cust.contactName.trim();
    if (!hasCompany && !hasContactName) {
      return { skipped: true, reason: 'missing companyName and contactName' };
    }

    let query = hasCompany ? cust.companyName : cust.contactName;
    if (hasCompany && hasContactName) query = cust.companyName + ' ' + cust.contactName;
    if (!hasCompany && hasContactName && cust.phone) query = cust.contactName + ' ' + cust.phone;

    console.log('[autoBgCheck] starting structured bg check for jid=' + jid + ' query=' + query);

    const result = await runFullBackgroundCheck(query, {
      email: cust.email || undefined,
      context: [cust.contactName, cust.title, cust.country, cust.city, cust.address].filter(Boolean).join(' ')
    });

    const now = new Date();
    const rating = result.rating || 'C';
    const summary = result.summary || '';

    // 从 dimensions 提取结构化字段
    let companyName = cust.companyName || null;
    let country = cust.country || null;
    let industry = cust.industry || null;
    let companySize = null;
    let website = cust.website || null;

    if (result.dimensions && result.dimensions.length > 0) {
      const step1 = result.dimensions[0];
      if (step1 && step1.data) {
        if (step1.data.companyName) companyName = step1.data.companyName;
        if (step1.data.country) country = step1.data.country;
        if (step1.data.industry) industry = step1.data.industry;
      }
      const step2 = result.dimensions[1];
      if (step2 && step2.data && step2.data.url) website = step2.data.url;
      const step5 = result.dimensions[4];
      if (step5 && step5.data && step5.data.companySize) companySize = step5.data.companySize;
    }

    // 保存到 CustomerBackgroundCheck 表
    try {
      await prisma.customerBackgroundCheck.upsert({
        where: { contactId: cust.id },
        update: {
          companyName, country, industry, companySize, website,
          details: JSON.stringify({
            dimensions: result.dimensions.map(d => ({ step: d.step, name: d.name, status: d.status, summary: d.summary || '' })),
            sources: result.sources || [], completedAt: result.completedAt,
            score: result.score || null,
            chapters: result.chapters || [],
            executiveSummary: result.executiveSummary || '',
            recommendation: result.recommendation || ''
          }),
          riskLevel: rating, notes: summary, source: 'llm'
        },
        create: {
          contactId: cust.id,
          companyName, country, industry, companySize, website,
          details: JSON.stringify({
            dimensions: result.dimensions.map(d => ({ step: d.step, name: d.name, status: d.status, summary: d.summary || '' })),
            sources: result.sources || [], completedAt: result.completedAt,
            score: result.score || null,
            chapters: result.chapters || [],
            executiveSummary: result.executiveSummary || '',
            recommendation: result.recommendation || ''
          }),
          riskLevel: rating, notes: summary, source: 'llm'
        }
      });
    } catch (bgcErr) {
      console.warn('[autoBgCheck] save CBC failed:', bgcErr.message);
    }

    // 更新 Customer.bgReport 和 bgUpdatedAt
    try {
      await prisma.customer.update({
        where: { id: cust.id },
        data: { bgReport: summary, bgUpdatedAt: now }
      });
    } catch (updErr) {
      console.warn('[autoBgCheck] update customer failed:', updErr.message);
    }

    // WebSocket 通知前端刷新
    if (io) {
      io.emit('customer:bgcheck:done', { jid, rating, summary: summary.slice(0, 200) });
    }

    // Phase 3: Auto BANT scoring after background check (fire-and-forget)
    try {
      autoBantScore(jid, io).catch(e => console.warn('[autoBgCheck] BANT follow-up error:', e.message));
    } catch (e) {
      console.warn('[autoBgCheck] BANT trigger error:', e.message);
    }

    console.log('[autoBgCheck] done for jid=' + jid + ' rating=' + rating);
    return { skipped: false, customerId: cust.id, rating, summary, jid };
  } catch (err) {
    console.error('[autoBgCheck] error:', err.message);
    return { skipped: true, reason: err.message };
  }
}
