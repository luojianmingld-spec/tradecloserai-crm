...
import { chatComplete, getActiveProvider } from "./ai-client.js";
...
export async function detectLanguage(text, engine = "google") {
  ...
  try {
    const langResponse = await chatComplete(
      [
        {
          role: "system",
          content:
    ...
    const lang = langResponse.trim().toLowerCase();
...
export async function translateText(text, sourceLang, targetLang, engine = "google", userId = null) {
  ...
  try {
    ...
    const systemPrompt = `You are a professional translator for international trade emails. Translate the given email text to ${targetName}. ${sourceInstruction}.

Rules:
1. Return ONLY the translated text, no explanations, no quotes wrapping the output
2. Preserve the original paragraph structure with line breaks exactly
3. Do NOT output >, >>, >>> quote prefixes that were in the source; output clean paragraphs only
4. Translate email headers like "On Mon, Sep 22, 2025, X wrote:" into natural Chinese like "2025年9月22日（周一），XXX 写道："
5. For business/trade terms use standard Chinese industry terminology (e.g. DDP=完税后交货, PI=形式发票, MOQ=最小起订量)
6. Keep numbers, currency symbols, URLs and email addresses as-is
7. Preserve line breaks between paragraphs
8. If text is already in ${targetName}, return it unchanged`;
    ...
    return { translated: trimmed, sourceLang, targetLang, cached: false };
  } catch (err) {
...
export async function autoTranslateMessage(message, userId) {
  try {
    ...
    if (settings.translationEnabled !== "true") return null;
    if (!message.content || message.messageType !== "text") return null;
    ...
    if (effectiveTarget === "auto") effectiveTarget = "zh";

    if (sourceLang === effectiveTarget) return null;

    const result = await translateText(message.content, sourceLang, effectiveTarget, engine, userId);
