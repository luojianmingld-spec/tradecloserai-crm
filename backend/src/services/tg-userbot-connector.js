...
function registerUpdateHandler() {
  if (!client) return;
  console.log('[TG-UB] Registering update handler (raw mode)...');
  // 先测试: 不带 filter，记录所有收到的 update 类型
  let updateCount = 0;
  client.addEventHandler(async (update) => {
    updateCount++;
    if (updateCount <= 100) {
      // Try different ways to identify the update type
      const type1 = update?.className;
      const type2 = update?.CONSTRUCTOR;
      const type3 = update?.constructor?.name;
      const state = update?.state;
      console.log('[TG-UB] Update #' + updateCount + ': type1=' + type1 + ' type2=' + type2 + ' type3=' + type3 + ' state=' + state);
    }
    try {
  ...
  console.log('[TG-UB] Update handler registered. Client connected:', client.connected);
}
